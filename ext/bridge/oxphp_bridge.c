...
static inline void check_cancelled_c(void) {
     ...
     * gone is the one cancellation that often must not: ending a request here
     * is a longjmp, and a longjmp never re-enters the VM, so no `finally` on
     * the way out is ever reached. In worker mode the thread outlives the
     ...
     * `finally` has nothing that does the same for it. The SAPI's answer: a
     * request that called ignore_user_abort(true) writes on, and so does any
     * worker-mode request that is not streaming; a worker-mode stream, or a
     * request outside worker mode, that did not make that call is still ended
     * here, as PHP ends it. With no callback registered the old answer
     * stands. */
