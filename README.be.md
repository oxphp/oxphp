...
- **Дэкаратары на аснове атрыбутаў** — перахоп выклікаў функцый/метадаў праз атрыбуты PHP 8+ з нулявым накладным коштам для недэкараванага коду; падтрымка `TARGET_FUNCTION`, `TARGET_METHOD`, `TARGET_CLASS`
...
- **Мультыплексаванне файбераў** — кожны воркер апрацоўвае некалькі адначасовых запытаў праз PHP 8.4 Fibers; `oxphp_sleep()` і `oxphp_async_await()` перадаюць файбер замест блакіроўкі воркера
...
- **Стрымінг SSE** праз аўтавызначэнне `Content-Type: text/event-stream` або `oxphp_stream_flush()` — кааператыўны з мультыплексаваннем файбераў
...
- **Абмежаваная чарга запытаў** з адмовай 529 пры перапаўненні
...
| `QUEUE_CAPACITY` | `PHP_WORKERS * 128` | Памер абмежаванага канала; 529 пры перапаўненні |
...
| **Built-in PHP Profiler** | Нізканакладнае прафіляванне праз атрыбуты дэкаратараў (`#[Timer]`, `#[Span]`), інтэграванае з метрыкамі сервера і трасіроўкай |
...
| ~~**Fiber Multiplexing**~~ | ✅ Рэалізавана — кожны воркер апрацоўвае некалькі адначасовых запытаў праз PHP 8.4 Fibers; `oxphp_sleep()` / `oxphp_usleep()` і `oxphp_async_await()` кааператыўна перадаюць файбер |
