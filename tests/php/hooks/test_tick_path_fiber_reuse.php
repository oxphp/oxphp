...
// the fiber that was on the free list, so the inner request taken first runs on a
// fresh one and, by completing, hands it back — the other then starts on a
// recycled fiber from inside the tick. A build that treats "recycled" as "resume"
// installs that fiber's never-written state snapshot over the request's own,
// which costs the second one its header list and status, and normally takes the
// worker down with a SIGSEGV on the second header() call.
$socks = [];
foreach ([1, 2] as $i) {
...
// one pass, so the inner request taken first runs to completion and reaches the
// free list before the other is created. That is a property of the loop, not of
// this test — put a suspend point in fixture_inner_state.php and the first would
// still be in flight when the second starts, which would leave this test passing
// while covering nothing. Keep the fixture free of awaits, sleeps and socket
// reads.
//
// Which of the two is taken first is not this test's to choose: each connection
// is read by a task of its own, and a request joins the worker's queue once its
// headers have been read, not in the order its connection was opened. So the
// pairing is compared without an order.
...
$t->assertTrue(
    ...
    isset($m1[2], $m2[2]) && $m1[1] === $m2[1] && abs((int) $m2[2] - (int) $m1[2]) === 1
);
