 ...
 *
 * Needs ASYNC_WORKERS >= 2: the holder and the cancelled task have to run on
 * different threads. The holder spins rather than sleeps: a sleep parks its
 * fiber, and a pool thread whose fibers are all parked takes the next task from
 * the queue — the cleanup would then re-enter the mutex on the holder's thread,
 * which is a DeadlockException, not a wait.
 */
...
$busy = oxphp_async(function () use ($m, $marker) {
    try {
        ...
        try {
            ...
            file_put_contents($marker, get_class($e) . ': ' . $e->getMessage());
        }
    }
});
...
while ((($outcome = @file_get_contents($marker)) === false || $outcome === '') && microtime(true) < $deadline) {
    usleep(50_000);
}
@unlink($marker);
if ($outcome === false || $outcome === '') {
    echo "FAIL: the cancelled task's cleanup never got the lock\n";
    return;
}
if ($outcome !== 'acquired') {
    echo "FAIL: the cancelled task's cleanup gave up on the lock instead of waiting for it: $outcome\n";
    return;
}
