...
// A request that is ended by the write path leaves userland's `finally` unrun.
//
...
// only from inside it. For a request that is ending anyway that costs nothing —
// the frames it abandoned are walked afterwards and their locals released, so
// even a destructor still runs, late. `finally` has nothing that does the same
// for it, and in worker mode the request is not what is ending — the mark it
// left behind outlives it. The worker carries on, and whatever the
// abandoned request had marked on it is still marked, for every request after
// it, for the rest of that worker's life. Code that guards a render against
// containing itself is then holding a guard against a render that finished
// minutes ago, and refuses the real one.
//
// It is silent and it is permanent: nothing is logged, no status changes, and
// no later request clears a static it did not set. The visible effect is that
// the worker quietly stops rendering the guarded thing, and the only way to see
// it is to go and look at the mark — which is what this does.
//
...
$t = new TestCase('cancel_skips_userland_cleanup', 'fibers');
...
$staged = write_cancel_stage($t, 'guarded write', '?key=' . $key, 'fixture_cancel_skips_finally.php');
...
if ($staged) {
    ...
    // It resumes out of the park and writes, and the write is where the worker
    // ends it. Checked separately from the mark so that a run in which the
    ...
    // The second edge. Polled rather than read once: the worker finishes the
    // request after the write ends it, and this request is parked in the poll's
