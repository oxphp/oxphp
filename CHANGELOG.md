...
- **`oxphp_async_await_all()`, `oxphp_async_await_race()` and `oxphp_async_await_any()` no longer leak one reference per result.** Each of the three materialized a promise's result in a temporary slot and then copied it into the array they return. The copy takes a reference of its own, so the temporary's reference had to be released afterwards — and none of the three released it. Every string, array or object delivered through them therefore stayed alive with one reference too many: unreachable from PHP, never collected, and reclaimed only when the request's allocator pool was torn down at the end of the request. In traditional request-per-script serving that is a bounded overshoot within a single request. In worker mode, where a request can live for the lifetime of the worker, it is unbounded growth proportional to how many results those functions delivered — a fan-out loop awaiting sizeable payloads leaks the full payload on every iteration. Measured with an identical array read, a result's reference count came back as 3 against a baseline of 2, exactly one reference too many, for all three functions; it is now 2. Results that are not reference-counted (`int`, `float`, `bool`, `null`) were never affected, and neither was `oxphp_async_await()` — the single-promise call writes its result straight into the return slot with no copy, which is why the defect never showed there. Nothing observable to a script changes: return values, keys and exception behaviour are identical, and the released reference is never the last one, so no destructor runs any earlier than before. One measurement caveat: with `opcache.jit=tracing` enabled, `memory_get_usage()` in a hot await loop can still show growth from a separate, unrelated defect that affects the single-promise call as well, so byte-level probes under JIT will not show this fix cleanly — reference counts and `opcache.jit=off` will.
