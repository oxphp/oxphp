...
- **HTTP Range requests for static files (RFC 9110 §14).** Static responses now advertise `Accept-Ranges: bytes`; a GET with a single-range `Range` header (`bytes=N-M`, `bytes=N-`, `bytes=-N`) receives `206 Partial Content` with `Content-Range`, enabling `<video>`/`<audio>` seeking, resumable downloads (`curl -C -`, `wget -c`), and partial PDF loading. Unsatisfiable ranges return `416` with `Content-Range: bytes */<size>`. `If-Range` is honored with strong ETag comparison and exact-date `Last-Modified` comparison. Multi-range requests fall back to the full `200` response (no `multipart/byteranges`), `206` responses are never compressed, and ranges apply only to static files — never to PHP responses, matching nginx + FastCGI behavior.

### Changed

- Static-file ETags are now strong (`"<size>-<mtime_hex>"`) instead of weak (`W/"…"`). `If-Range` requires strong comparison per RFC 9110, and size + mtime fully identify a static file's bytes — the same reasoning behind nginx's strong static ETags. Clients revalidate once after the upgrade (the cached weak tag no longer matches), then cache as before.
