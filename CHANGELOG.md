...
- **Worker mode with a dynamic pool: retiring a worker now retires it, and the process can still stop afterwards.** With `PHP_WORKERS=MIN:MAX`, the scale manager retires a worker that has been idle past `PHP_WORKERS_IDLE_SECONDS` — but a worker-mode thread only ever left on the request queue closing, and never looked at the flag the scale manager had raised for it. Two consequences, neither of them visible in the log. The thread went on taking requests off the shared queue as though nothing had happened, so the pool never actually shrank while reporting that it had: `oxphp_workers_current` dropped by one at every scale-down, and the count it published diverged further from the number of threads serving with each one. And the process could no longer shut down: the retirement schedules a join for the thread, the queue closes only in the very last stage of shutdown, and that stage is reached only after everything the server scheduled has finished — so `SIGTERM` never completed, and orchestrators killed the container at their grace period instead. A single retirement over the whole life of the process was enough, which on a production pool happens by itself after the first lull in traffic. A worker-mode thread now reads that flag, and leaves the first time it finds itself with nothing in flight — so whatever it was serving still runs to its real response, and once the worker reaches that moment the count is honest again, the thread is released and shutdown completes. A worker holding a request that never ends, such as an open stream, is the boundary of that: it does not reach an idle moment and so does not retire, and its thread still stops the way it always did, when the request queue closes. A completed retirement is logged (`Scale-down: retired worker thread stopped`) next to the line that begins it, because a retirement that never completes has to be distinguishable from one that did. Static pools (`PHP_WORKERS=N`) never retire anyone and are unaffected, as are traditional, framework and SPA serving, where a worker that can be retired already read its flag.
