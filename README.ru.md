...
- **HTTP-кеширование** с ETag, Last-Modified и поддержкой 304 Not Modified для статических файлов
...
- **Журнал доступа** в виде структурированного JSON через tracing (уровни: `all`, `error`, выключен через `ACCESS_LOG`)
...
- **Настраиваемый Tokio runtime** — автоматическое определение потоков (CPU / 2, мин. 1) или явное задание через `TOKIO_WORKERS`
...
- **Режим воркера** — постоянные PHP-процессы с мягким сбросом между запросами, автоматическая рециклизация по числу запросов или памяти, метрики Prometheus по каждому воркеру
...
| `TOKIO_WORKERS` | `0` (CPU / 2, мин. 1) | Потоки асинхронного I/O Tokio; `0` = авто, `1` = однопоточный, `N` = многопоточный |
| `EXECUTOR` | `sapi` | Исполнитель PHP: `sapi` (настоящий PHP) или `stub` (режим тестирования) |
| `PHP_WORKERS` | `0` (CPU / 2, мин. 1) | Режим пула воркеров: `N` = фиксированный пул, `MIN:MAX` = динамическое масштабирование, `0` = авто |
| `PHP_WORKERS_IDLE_SECONDS` | `30` | Таймаут простоя перед завершением динамического воркера (только в динамическом режиме) |
| `QUEUE_CAPACITY` | `PHP_WORKERS * 128` | Размер ограниченного канала; 503 при переполнении |
| `DRAIN_TIMEOUT_SECONDS` | `30` | Таймаут ожидания завершения запросов при плавной остановке (секунды) |
...
| `STATIC_CACHE_TTL` | `30d` | TTL кеша статических файлов. Форматы: `30s`, `5m`, `2h`, `30d`, `1w`, `1y`, число секунд (`3600`) или `off` для отключения |
| `COMPRESSION_ENABLED` | `true` | Включить сжатие Brotli; отключить значениями `false`, `0` или `off` |
| `ACCESS_LOG` | *(выкл.)* | JSON-журнал доступа: `all` (все запросы), `error` (только 4xx/5xx), пустое/не задано = выключен |
| `MAX_CONNECTIONS` | `10000` | Максимальное количество одновременных соединений |
| `WORKER_FILE` | *(не задано)* | Путь к PHP-скрипту воркера (относительно `DOCUMENT_ROOT`); включает режим постоянных воркеров |
| `WORKER_MAX_REQUESTS` | `0` (без ограничений) | Макс. запросов на воркер до рециклизации; `0` = без ограничений |
| `WORKER_MAX_MEMORY_MIB` | `0` (без ограничений) | Макс. память (МиБ) на воркер до рециклизации; `0` = без ограничений |
...
- **Настраиваемый Tokio runtime** — по умолчанию авто (CPU / 2, мин. 1) через `TOKIO_WORKERS=0`, `1` = однопоточный, `N` = многопоточный для высоконагруженных сценариев
...
- **Режим воркера** — постоянные PHP-процессы с мягким сбросом между запросами; воркеры вызывают `oxphp_worker($handler)` в цикле, сохраняя состояние начальной загрузки (автозагрузчики, подключения к БД) между запросами
