...
| `STATIC_MAX_AGE` | `30d` | `Cache-Control: max-age` для статических файлов (`30s`, `5m`, `2h`, `30d`, `1y`, `off`). Заменяет устаревшую `STATIC_CACHE_TTL`. |
| `STATIC_REVALIDATE` | `off` | `on` — включает проверку mtime для кеша контента в памяти. Заменяет устаревшую `STATIC_CACHE` (где `off` означал то же самое). |
