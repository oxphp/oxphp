...
impl Metrics {
    ...
    pub fn to_prometheus(&self) -> String {
        ...
        let _ = writeln!(
            out,
            "# HELP oxphp_admission_wait_wasted_total Requests that waited out their whole wait budget while the pool began no request at all. Measured as work started, not as entries leaving the queue: a worker clearing requests the deadline already answered moves the queue without serving anybody. A subset of oxphp_admission_refused_total{{reason=\"wait_timeout\"}}: those waits lost a race for a worker, these never had one to lose. Read only while oxphp_admission_wait_budget_us is at its ceiling: over a window the server has already shortened, \"the pool began nothing\" is a statement about the window rather than about the pool — a healthy pool of four workers on quarter-second handlers starts one request every 62ms and would land outside most of them. The server reads the abandonment ratio only once the pool has started enough requests for that ratio to mean anything, and a pool that starts nothing never reaches that count behind a queue, so it never reaches a decision that could shorten it, holds its ceiling on its own and keeps this armed — whether or not its workers are still answering requests whose clients had already gone, which moves oxphp_abandoned_work_total without starting a thing. A self-calling pool does start requests, and once its callers are less patient than its handlers and there is still work queued behind them, the budget comes down on its own and this counter goes quiet through it: what is left to count is armed by the budget it arrived on, so the arrivals from before the drop go on being counted for up to a whole ceiling after it and then nothing is. Measured at two halvings within seconds, after which it counted nothing that arrived. Nothing takes over from it there: a budget under its ceiling with oxphp_abandoned_work_total climbing is the controller's own input and its own output, and an ordinary overload of impatient clients reads exactly that — measured on a pool with no self-call in it at all, the budget down to an eighth of its ceiling with abandoned work climbing and this counter all but still, 0 in that run. The wedge readings built on oxphp_busy_workers at 0 do not apply either, because a self-caller's workers are busy inside the handler that is doing the waiting; they do still name a pool that stops taking work off its queue while an overload already had the budget down. Sustained non-zero therefore means waiting is buying nothing at full length — shorten QUEUE_WAIT_TIMEOUT_MS; where the server shortens it first it is not because it recognised a self-call, so a pool whose callers are patient keeps its ceiling and this counter is what names it. Setting it to 0 drops the wait for a queue slot but not the wait inside the queue, so it needs a QUEUE_CAPACITY small enough to be full on a pool that picks nothing up. The classic cause is an application calling back into this same server over HTTP, where the inner request waits for a worker the outer one is holding; a pool whose handlers all outlast the budget produces it too."
        );
