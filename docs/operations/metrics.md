...
This measures time spent waiting — for admission and then in the queue — with the script's own execution time subtracted, so it answers "how long before a worker picked this up" rather than "how long did the request take". A request no worker picked up is not recorded here, whichever way it was answered: a `529` refused for overload, counted in `oxphp_admission_refused_total`, and a `499` for a client that had already gone, counted in `oxphp_request_cancelled_total{reason="client_abort"}` and in no refusal series at all.
