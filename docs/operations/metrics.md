...
| `oxphp_admission_refused_total` | counter | Requests answered without reaching a worker. Label: `reason` — `wait_timeout` (waited the full `QUEUE_WAIT_TIMEOUT_MS`; give the pool more headroom — but read `oxphp_admission_wait_wasted_total` first, because where the pool began nothing at all during those waits the remedy is the opposite one), `waiting_full` (already `QUEUE_MAX_WAITING` requests waiting, raise that or `MAX_CONNECTIONS`), `waiting_bytes` (the bodies already parked fill `QUEUE_MAX_WAITING_BYTES`, so this request's body had nowhere to sit — raise that, or lower how much a client may upload), `queue_full` (`QUEUE_WAIT_TIMEOUT_MS=0`, waiting is off), `shutting_down` (the drain deadline passed while the request was still waiting for admission), `pool_unavailable` (no worker thread is left to hand the request to — the pool is gone, not busy). Only the first four are overload and answer 529; `shutting_down` answers 503 like the rest of graceful drain, and `pool_unavailable` answers 500. Alert on overload with those four specifically: the metric as a whole also moves on a restart. Excluded from `oxphp_queue_wait_us` |
| `oxphp_admission_wait_wasted_total` | counter | Requests that waited out `QUEUE_WAIT_TIMEOUT_MS` while the pool began no request at all. What counts is work started, not entries leaving the queue — a worker clearing requests their deadlines already answered moves the queue without serving anybody, and reading that as progress would silence this counter exactly where it is needed. A subset of `oxphp_admission_refused_total{reason="wait_timeout"}`, separating a wait that lost a race for a worker from one that never had a race to lose. Sustained non-zero means waiting is buying nothing on those requests: shorten `QUEUE_WAIT_TIMEOUT_MS`, so they are refused sooner instead of holding a connection for most of the budget. `0` is not the shortest version of that but a different setting: it drops the wait for a queue *slot* and leaves the wait *inside* the queue unbounded, so a request that finds a slot free is queued with no deadline at all. On the pool this counter is naming — one picking nothing up — that is the one setting under which such a request is never answered, so `0` belongs with a `QUEUE_CAPACITY` small enough that the queue is full by the time it matters. The classic cause is an application calling back into this same server over HTTP — the inner request needs a worker the outer one is holding while it waits for the inner one, so no amount of waiting can succeed — but it is not a self-call detector: a pool whose handlers all run longer than the budget frees nothing during a wait either, and reads the same. What separates them is how long the handlers take, and in worker mode `oxphp_worker_request_duration_us` measures exactly that — handler time, with the queue wait excluded — so handlers sitting below the budget while this counter climbs means the waits are not merely queued behind slow work. Do not read `oxphp_request_duration_us` for it: that one is end-to-end over every request including static files, and it counts these refusals themselves, each of which contributes very nearly a whole budget, so it rises as this counter does. Outside worker mode there is no handler-time series, and the question has to be answered from the application's own timings |
| `oxphp_queue_depth` | gauge | Requests sitting in the worker queue right now: admitted, not yet picked up by a worker. An entry leaves only on a pickup, so this counts requests already answered `529` on their deadline alongside ones still waiting for a worker — a depth above `oxphp_pending_requests` is that difference. Present only for the SAPI executor, which is the only one with a queue |
...
| `> 0` | `> 0` and **not** falling, with `oxphp_busy_workers` at `0` | the pool has stopped taking work off the queue while admission still has room. This is a wedge in its first phase: arrivals are still being admitted and each is answered `529` when its own budget runs out, wherever one is configured, so `oxphp_admission_wait_wasted_total` climbs with every one of them — no request began running while any of them waited. Nothing else gives it away: no worker is busy, and the refusals on their own read like overload |
