...
### Sizing the async runtime

`TOKIO_WORKERS` defaults to half the available cores, and the half it leaves is not idle — it belongs to the PHP worker pool. Those threads are not the runtime's, and the two sets compete for the same cores, so the runtime cannot be widened without taking cycles from the application.

Raising it to the full core count is a loss wherever the pool is already sized near that count. Measured on a 4-core machine with a 4-worker pool, serving compressed responses: going from 2 runtime threads to 4 cost 13% of the rate on a 1.6 KB JSON body and 26% on a 5 KB one, and burned more CPU doing it. The threads were oversubscribed, and the scheduler paid for it.

It is worth raising where the balance genuinely differs: a pool small relative to the machine, or work that is I/O-bound rather than PHP-bound — proxied upstreams, static files, streaming responses — where the runtime does the work and the pool mostly waits. Whenever you change it, watch the pool as well as the rate: `oxphp_busy_workers` against `oxphp_workers_current` tells you which side the cores went to. A rate that rises while the pool gets busier is a real gain; a rate that rises while the pool empties usually means the work moved, not that more of it got done.

Both this and `PHP_WORKERS` default to half the machine, so their sum lands on the core count. Change one and the sum moves — which is what makes either one worth measuring rather than guessing.
...
| `QUEUE_WAIT_TIMEOUT_MS` | `1000` | The longest a request may spend waiting for a PHP worker before it is rejected with 529 — a ceiling rather than a fixed value. One deadline, stamped on arrival, covering both waits a request can face: for a slot in the queue, and inside the queue for a worker. The server shortens the budget below this ceiling on its own while the work the waiting admits is completing for clients who have already left, and lengthens it back once the queue drains; `oxphp_admission_wait_budget_us` publishes what is in force and `oxphp_admission_wait_budget_ceiling_us` what was configured. The shortening is not gentle and is worth knowing the size of before choosing a ceiling: the budget is **halved** each time a quarter or more of what the pool started went to clients who had already gone, measured over a window of at least eight started requests rather than a fixed interval, and it can go down to a floor of a sixty-fourth of this value — 15.6 ms at the default — or 10 ms, whichever is larger, though never above the ceiling itself. It walks back up in eighths of the ceiling, and only while the queue is observed empty and less than a quarter of what the pool started went to departed clients — so the ordinary trickle of clients closing a tab or timing out mid-request does not hold it down, and a server whose queue never empties stays on the shortened budget until the load lets up. An operator who has set 1000 here should expect 15 to be in force during an episode, and an alert on the wait should be written against the pair of series rather than against this number. The deadline is enforced from both ends: the request is answered with a 529 when it passes, and a worker that reaches a request past it refuses rather than executes it. Enforcing it only at pickup would make the budget a rule about which requests run rather than about how long anyone waits — a queued request nobody picks up would wait indefinitely, which is what an application calling back into this same server produces. It does not bound how long the handler then runs: a request a worker started inside the budget is left alone. At most `QUEUE_MAX_WAITING` requests wait at once; past that, requests are rejected immediately. `0` = reject immediately whenever the queue is full. Lower it when the application calls back into this same server over HTTP — the inner call needs a worker the outer one is holding while it waits for the inner one, so the wait is spent for nothing however long it is given; `oxphp_admission_wait_wasted_total` counts those waits, but only while the budget is at this ceiling. Whether it stays there is not a property of the self-call: the budget comes down on work finishing for clients who have already left, which the server can only read from a queue it finds non-empty when it looks and from enough started requests behind it — so the same self-caller keeps its ceiling wherever its own callers wait for it, an internal cron or a sidecar on a generous timeout being the ordinary case, and comes down only where they do not *and* those two conditions hold. Where the budget does come down, that counter goes quiet, so a `0` is not by itself the absence of this pattern — or when a load balancer in front has a shorter timeout of its own. `0` shortens nothing here: it removes the wait for a queue slot and not the wait inside the queue, so unless `QUEUE_CAPACITY` is small enough for the queue to be full, the inner call is admitted to a queue nothing is consuming and is never answered at all. A client that closes the connection mid-wait gives its place back immediately, on HTTP/1.1 as well as HTTP/2, so a balancer that times out and closes does not fill the waiting set with attempts it has already abandoned. A request already in the queue when its client closes keeps its place until a worker reaches it or the budget runs out, and is then answered `499` rather than counted as an overload refusal. What the server cannot see is a client that stops waiting *without* closing — it holds its place until it is admitted or the budget runs out, and if a worker frees up first its script runs for nobody — so keeping the budget below the timeout of anything in front of it is still worth doing. The server's own shortening does not cover that case either: it is driven by clients who close, so a population that stops waiting without closing produces no signal and leaves the ceiling in force |
...
The default cannot compute this — service rate is not known at startup — so it is deliberately generous. That fits fast handlers, where the pool drains a deep backlog well inside the budget, and is far too large for slow ones: with 8 workers, a 200 ms handler and the default 1 s ceiling — the budget in force whenever the pool is not shedding — only about 40 requests can be admitted in time while the default parks up to 1024.

```bash
QUEUE_MAX_WAITING=40
```

Setting it near `W × B / T` turns the surplus into an immediate 529 rather than a 529 a second later. Shortening `QUEUE_WAIT_TIMEOUT_MS` achieves the same by the other term — the two trade against each other, and on a slow handler the smaller budget is usually the better lever, because it also bounds how long a client waits for the refusal. `B` here is the budget in force, which under overload is the one the server has shortened to rather than the configured ceiling — so read `oxphp_admission_wait_budget_us` before sizing the waiting set from a number in a config file.
...
> **Note:** A client that closes its connection before the server has answered is logged at `debug`, not `error`. It is the commonest thing a public listener sees — a cancelled fetch, a closed tab, a proxy whose read timeout fired — and it is not a failure of the server's, so at the default `LOG_LEVEL` it is invisible. Count them with `oxphp_request_cancelled_total{reason="client_abort"}`; raise `LOG_LEVEL=debug` to see the individual connections. Timeouts the server itself enforces (`HEADER_TIMEOUT_SECONDS`) stay at `warn`, and every other connection failure stays at `error`.
...
When APM is enabled, OxPHP automatically hooks 33 internal PHP functions (PDO, mysqli, cURL, Redis, Memcached, file I/O) to create child spans. The `oxphp_apm_*()` PHP functions are registered regardless of whether APM is enabled — when disabled, the span functions are no-ops returning sentinel values. `oxphp_apm_trace()` is the exception: it still runs its callback and returns the callback's value, so disabling APM never changes what your application computes.
...
|----------|---------------|
| `sleep` | Native `sleep()` and `usleep()` suspend the current fiber exactly like `oxphp_sleep()`/`oxphp_usleep()` — including the contexts where those two cannot suspend either and wait on the worker thread instead (see below) |
...
**Installed is not the same as in effect.** What that list reports is which builtins had their handlers replaced, which happens at startup and does not depend on the routing mode. Where there are no fibers at all the replacements delegate to the handlers they replaced, so a traditional-mode server and a `php` CLI process both report `["sleep","streams"]` while behaving natively throughout. Read the list as "this process is set up for the hooks", and the routing mode as whether anything can use them. Within `streams` it is narrower still: the socket-read part can fail to install on its own, on a platform where the stream ops table cannot be made writable, while `stream_select()` and the database entry points stay hooked. The category is reported either way, and the only signal for that partial failure is a line in the server log saying socket reads stay blocking. That line follows PHP's error logging: with `error_log` unset, which is the default, it reaches the server's own log as a `warn`, and `LOG_LEVEL=error` hides it. The field says `streams` either way. The same is true of the unrecognised-category line above.

Hooks take effect inside worker-mode request fibers and async task fibers. Outside a fiber (traditional/framework/SPA request context, CLI) the original native behavior is preserved, including argument validation errors. With the `sleep` hooks enabled, third-party code calling `sleep()` stops pinning the worker thread wherever the fiber can be suspended — which is not everywhere, as the next paragraph sets out — and needs no code changes either way. Cancelling an async task during a hooked sleep unwinds it with `OxPHP\Async\AsyncException`, and a hooked `sleep()` that suspends returns `0`, the signal-interruption return value of the native builtin arising only on the blocking path below.

**Where the `sleep` hooks do not suspend.** Where a fiber switch is refused, the hooked `sleep()`/`usleep()` hand the call to the builtin they replaced, which waits on the worker thread and returns what it returns — so a `sleep()` cut short by a signal there gives back the seconds it did not sleep, as it does outside a fiber and as it would with the hooks off. (The argument check is the hook's own either way: it rejects anything above `4294967295`, which the unhooked builtin accepts and truncates.) Switching is blocked wherever leaving the current frame would be unsafe, which covers more than a list can close: inside a `declare(ticks)` handler and inside pcntl's signal dispatch, both upstream PHP's own doing; while a request's input is being built; inside a `FILTER_CALLBACK` reached from `filter_input_array()` given a per-field definitions array, described in the note under `$_GET` in [Superglobals](../php/superglobals.md); and inside the teardown walks that run destructors after a request has come apart — after a fatal, or after a write to a request that was already cancelled — so a `__destruct()` that sleeps to back off does it holding the thread. The ordinary end of a request is not one of these: destructors and shutdown functions run there with switching allowed. A fiber a userland scheduler started — AMPHP, Revolt, or a bare `new Fiber` — is a different case rather than another entry in that list: switching is not blocked there, and the scheduler suspends and resumes its own fibers normally, but OxPHP cannot resume a context it does not own, so the hook stands aside for the same reason the `streams` hook does. `oxphp_sleep()` and `oxphp_usleep()` take their blocking path in every one of these places, so reaching for them instead is not a way around it. That last case is about a fiber started *inside* a request — by `new Fiber` directly, or by a scheduler the application drives there — and not about the shape of a whole deployment: a worker-mode request is itself a fiber OxPHP drives — which is why Revolt refuses to run its loop from inside one, as the note below on the `streams` hook says — so the hooks apply to the request's own fiber normally. What does sleep on the worker thread is the code inside a fiber the application started itself, and that share of the load has to be sized as if the hook were off.
...
- **One connection shared between concurrent fibers is safe for the clients OxPHP guards, and gains nothing.** This is the normal shape of a worker-mode application rather than an edge case: WordPress, Laravel and Symfony open their database and cache clients once when the worker boots and hand the same ones to every request, and short of rewriting their data-access layer they cannot be asked for one connection per fiber. A client protocol is a sequence of exchanges — write a command, read the answer — with nothing on the connection marking where one ends, so a fiber parked on a read is parked in the middle of one, and a second fiber's command landing there breaks the protocol. The two clients fail at that differently, and neither well: mysqlnd tracks its connection state and refuses the command from it, before sending anything, while phpredis has no such check and the two fibers read each other's replies — one request's data returned to another with no error raised. A fiber therefore claims a connection before using it, at both of the levels this has to happen at: the socket ops, which keep the bytes in order, and the client entry points of PDO and mysqli, since mysqlnd's refusal comes before any I/O and nothing at the socket can be reached in time to prevent it. Another fiber reaching the same connection waits for it to be given up at the client level, where nothing but the connection's identity is held; at the socket level it does not wait but is refused, the way a socket timeout is refused, because a fiber suspended inside an operation on someone else's stream would be holding a pointer its owner may free. phpredis is guarded at the client level too, method by method, for exactly that reason. What the client level claims is the connection itself and not the PHP object holding it, so a persistent connection reached through several PDO objects counts as one, and a connection opened with `PDO::connect()` — which returns the driver's own subclass rather than a `PDO` — is covered like any other. Read the hook's gain, then, as belonging to connections a fiber opens for itself: an async task making its own HTTP or database calls, a request opening its own client. What a shared connection gets is the worker thread back while it waits, so other requests can run the work that is not on that connection; its own exchanges run one after another, exactly as they do with the hook off. Four boundaries are worth knowing. **First**, a fiber that queried once keeps the connection to the end of its request, because the end of the request is the first moment certainly past the end of an exchange — and it keeps it while parked on anything else as well, so a request that has queried and then waits on work of its own that needs the same connection is waiting for itself, and the two come apart on the bound below instead of proceeding. **Second**, waiting is always bounded: by the smaller of `max_execution_time` and `default_socket_timeout`. Set neither and that bound is 30 seconds, since a server SAPI takes the engine's defaults — 30 for the first and 60 for the second; it is `default_socket_timeout`'s 60 only where `max_execution_time` is 0. `max_execution_time` is read as the request currently has it, since `set_time_limit()` is how a request states how long it may run; `default_socket_timeout` is read as the process started with, because it is the default deadline of a socket *operation* and a request narrowing it for a call of its own — a common thing for a library to do, and to leave behind — must not shorten this bound for the requests that follow it on the same worker. Past the bound the call falls back on unguarded behaviour and says why in the server log — for PDO and mysqli it is handed to the client, whose own refusal of a command issued mid-exchange is the error the application already handles, while for phpredis, which has no such refusal and would instead read someone else's reply, it raises a `RedisException` and sends nothing. A socket-level conflict has no bound of its own because it never waits: the operation fails at once the way a timeout does, so `stream_get_meta_data()` reports `timed_out`. That can cost more than the call, and on either side of an exchange: a client is free to read a refused operation as something worse than a failed command, and mysqlnd does so for both — a write that sends nothing marks the connection gone and closes it, a read that comes away empty marks it gone where it stands, and every later command on it is answered from that state without touching the wire. So what is lost is not the call but the connection, for as long as anything holds the handle. The client-level claim above is what keeps a guarded client's commands from reaching that refusal at all; a hand-written protocol on a raw socket, which has no equivalent, should treat a refused operation of either kind the way it treats a closed connection. Two fibers each holding what the other waits for therefore come apart on that bound instead of waiting on each other for good. Code running outside any fiber cannot wait on a claim at all, and only the socket level applies to it. In worker mode with `RUNTIME_HOOKS=streams` that is chiefly a destructor the worker loop runs between requests when it collects cycles, while other requests are parked. A socket operation it makes that would wait, on a connection whose holder is parked on it waiting for its reply, fails at once the way a timeout does, and sends nothing; at any other moment it goes ahead. The client level is not applied there, because its claim lasts to the end of the holder's request: it would refuse such code a connection the holder is not using at that moment, which is most of the time. What a refusal costs is the client's, as above. phpredis fails the call and the holder keeps its reply. mysqlnd either refuses the command from its own state or, when the holder's exchange left that state ready, has its write refused, closes the connection, and the holder's request ends as described under **Fourth**. A destructor is therefore a poor place to use a shared client at all. **Third**, some cases are deliberately left uncovered. A statement or result object kept across requests, which no claimed call precedes — `PDOStatement::execute()` on a statement prepared in an earlier request behaves as it does with no claim at all. What that costs is worth stating, because it is more than the call: such a call reaching the socket while any fiber is parked on the connection is refused there, and for mysqlnd a refusal is the connection rather than the call, as above — on the read that fetches the next row of an unbuffered result as much as on the write that sends the command. A protocol written by hand on a raw socket — write a command, suspend on something else, read the reply later — since the socket level only refuses while the holder is parked on the reply itself, and between those two points another fiber takes the connection over; the three clients above are covered here by their client-level claim, which a hand-written protocol has no equivalent of. A persistent connection is a case of its own and is covered rather than left out: `PDO::ATTR_PERSISTENT` keeps the driver's connection in a pool the whole worker shares, and PDO reaches that pool in two steps that do not survive being interleaved — it looks a pooled connection up before it connects, and registers what it built afterwards, so two constructors overlapping both miss the pool and the second registration replaces the first entry, closing that connection under whichever request is reading its reply on it. Overlapping is the ordinary case rather than a rare one, since `static $pdo ??= new PDO(...)` is reached by every request that arrives before the first connect finishes. So constructors asking for a persistent connection to one data source run one at a time per worker, which is the whole width of a pool, and the second one finds what the first registered. The data source is the one PDO resolves — an alias from `pdo.dsn.*` in php.ini counts as the DSN it names — and constructors to different ones do not, as a rule, wait for each other — so one stuck connecting to a server that accepts the connection and never answers does not hold up constructors to a healthy one. Two cases wait more rather than less. A DSN is compared only up to the first colon after its driver prefix — an IPv6 host is one way to have one — because PDO joins DSN, user and password into its pool key with colons and escapes nothing, so a DSN extended by `:…` can name the same pool entry as the one without. Sources that read the same up to that colon share one wait, a stuck one included: two IPv6 hosts in the same leading group of digits wait for each other as all constructors on a worker used to. And a `uri:` DSN, or a DSN passed as anything other than a string, cannot be resolved without repeating what PDO does with it, so such a constructor waits until no persistent constructor to any data source is running on its worker, and they all wait while it runs. It holds nothing while it waits, so it does not hold the others up; but while a constructor to a server that never answers is running it cannot find that moment, and when its wait is spent it goes ahead unserialised — with `uri:` or object DSNs, the stuck-server case above still reaches it. And one that names the stuck server itself holds every persistent constructor on its worker up for as long as its connect lasts, exactly as before. That wait is bounded like the ones above, and unlike them what it costs when it is spent is not an error the client raises: a constructor whose budget runs out goes ahead unserialised and says so in the server log, and two constructors overlapping that way replace each other's pool entry as before — which ends the request reading its reply on the connection replaced and, because a pooled handle is freed without regard to how many PDO objects still point at it, can end the worker process itself when one of those objects is later destroyed. A constructor inside a userland fiber scheduler's own fiber is the one place where that budget is spent at once rather than occasionally: such a context cannot be suspended, so a constructor that finds another one already running gives up immediately instead of waiting. One that finds the way clear still takes its turn like any other, and the fibers OxPHP drives still wait for it. The other step is the liveness check PDO makes before handing a pooled connection over: it is a ping, sent on the connection itself, and one sent while the fiber holding that connection is parked on it waiting for its reply is refused, so PDO reads it as dead, drops it from the pool and opens a second connection — the request mid-exchange keeps its own connection and its reply, but what the application meant to share is no longer shared. A connection another fiber holds is therefore reported alive without the ping, on the grounds that being used is what a live connection looks like — but only to a constructor that would leave it exactly as it is. Where a driver supplies no liveness check of its own — `pdo_sqlite` is one — PDO asks nothing at all before handing the connection over, and the answer stands in that place instead, so what follows holds whichever driver opened the connection. PDO does not stop at handing a pooled handle over: it writes the error mode and the autocommit flag onto it from the options it was given, defaults included when they are absent, and then applies the rest of those options to it as well. On a connection in use those writes are the holder's — its error mode changing under it, and for options the driver forwards, such as `PDO::ATTR_STRINGIFY_FETCHES`, the PHP types its remaining rows come back as. So the connection is shared only with a constructor that would write nothing new onto it, and that is two conditions rather than one. Every option it passes must be one PDO stores on the handle itself — `PDO::ATTR_ERRMODE`, `PDO::ATTR_CASE`, `PDO::ATTR_ORACLE_NULLS`, `PDO::ATTR_DEFAULT_FETCH_MODE` — and must pass the value the connection already has. The `PDO::ATTR_PERSISTENT` entry that names the pool is not counted among them, although PDO passes it to the driver like any attribute it has no case for: it sets nothing, and that trip is answered rather than sent to the connection. An option the driver has to be told about is refused whatever its value, `PDO::ATTR_AUTOCOMMIT` and `PDO::ATTR_STRINGIFY_FETCHES` included, because what a driver does with one is the driver's own — a field written on the connection, or a command put on the wire, which here would land inside the holder's exchange. The rule stands on whichever driver opened the connection, third-party ones included, so it is answered by where the option goes rather than by what any one driver does with it; asking for the value the connection already has does not change where it goes. A constructor that fails either condition is told the connection is dead and gets one of its own, rather than changing one in use, and the server log says so once a second. Note what that means for a pool key whose options carry `PDO::ATTR_EMULATE_PREPARES` or `PDO::ATTR_STRINGIFY_FETCHES`, as Laravel's do: it is shared between requests, and between fibers that construct before they query, but a constructor arriving while another fiber's request holds the connection opens its own. That is a longer window than the query it protects: a claim runs from the holder's first query to the end of its request, as above, not for the length of an exchange. A connection nothing is using is checked as before — including one whose holder has dropped its last handle, since the claim outlives the object it was taken for and what it then names is a connection nobody can send anything on — so one that really has died is still replaced. That check is a command on the connection like any other, and it is claimed like one: a query arriving through one of the clients above while it is in flight waits for it, within the same bound as any other contended call, rather than landing inside it. The cases listed as uncovered stay uncovered here too — what reaches the connection without passing a claimed call can land inside the check as it can land inside a query. **Fourth**, closing a shared connection while another request is parked reading it ends that request, with a `500` and a log line naming what happened. Claiming keeps two exchanges apart; it cannot keep a connection alive, and the parked request's reply is on a connection that no longer exists — so the honest answer is to end it rather than to hand back whatever the freed memory now holds. Which calls can do this depends on the client, and only one of them waits: `mysqli::close()` and `mysqli_close()` are claimed calls, so they wait for the holder and close once that wait is spent, while `Redis::close()` is claimed too but raises a `RedisException` rather than closing. Everything else closes at once — `fclose()` on a raw stream, and, importantly, PDO, which has no `close()` method at all: a connection is released by dropping the last reference to the handle (`unset($pdo)`, reassigning it, or letting it fall out of scope), and that path runs inside the engine's own object teardown where no claim is consulted. So the reconnect helper that reassigns a shared PDO handle ends the requests parked on the old connection, without the bounded wait its `mysqli` equivalent gets. The request is told as soon as the close happens rather than at its own read deadline, which for mysqlnd would be `mysqlnd.net_read_timeout` — a day out of the box. Applications that close a shared connection to force a reconnect — WordPress's `$wpdb->check_connection()` is the usual one — should expect the requests parked on it at that moment to fail rather than to return the wrong data.
- Running the `streams` hook underneath a userland fiber scheduler (AMPHP, Revolt) falls back to blocking I/O. A fiber started by such a scheduler runs on its own context, which OxPHP's scheduler cannot resume, so the hook detects this and takes the native path rather than corrupting either scheduler. This is about a fiber a userland scheduler starts *inside* a request. The request's own fiber is one OxPHP drives, and in worker mode it is a real `Fiber` — `Fiber::getCurrent()` inside a request returns it — so libraries that only need to tell concurrent requests apart work without any fallback. The same fact is why Revolt refuses to run its event loop from inside a worker-mode request; see the note on that in the changelog.
...
- Reach for `oxphp_sleep()` / `oxphp_usleep()` in code you write. They suspend the fiber in worker mode with no environment flag to set, and `oxphp_sleep()` accepts fractional seconds (`oxphp_sleep(0.25)`), a precision native `sleep()` (integer seconds) cannot express. What they do not buy is an exemption from the cases above: there they hold the worker thread exactly as the hooked builtins do.
- Enable `RUNTIME_HOOKS=sleep` for code you **cannot** edit — a framework or vendor library that calls native `sleep()`/`usleep()` directly. It is off by default, does nothing outside a fiber, and keeps the native return contract: a hooked `sleep()` still takes an `int`, and it returns `0` when it suspends and whatever the builtin returns when it falls back. The argument check is its own, as above.
