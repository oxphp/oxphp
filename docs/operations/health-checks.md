...
A pool can stop taking work off its queue while every thread it owns is alive: requests pile up, no worker is busy, and nothing it has queued is ever run. They are still answered once each has spent its own wait budget — a `529`, or a `499` where the client stopped waiting first — so from outside it does not read as a hang: what has stopped is the serving, not the answering, and the queue those refusals leave behind fills anyway, because only a pickup releases an entry. Every other signal reads healthy through it — the threads are all there, the plugins are all fine, static files are served normally — and a pool still in that state a minute later is not generally getting out of it on its own, which is why readiness carries it.
...
**Load.** An overloaded server stays `Ready`. Once the admission queue is full and the wait budget is spent, requests are refused with `529 Site is overloaded` — and a replica doing that is still answering, quickly, on a pool that is working. Taking it out of rotation would move its traffic onto its neighbours, and under an even overload that removes every replica in turn until the Service has no endpoints left: a degraded site becomes an unreachable one, with the load still there. Watch `oxphp_admission_refused_total{reason=...}` for it instead and add capacity — but read `oxphp_admission_wait_wasted_total` alongside `oxphp_admission_wait_budget_us`, because it counts only what arrived while that gauge was at its ceiling, and so goes still within a budget of the gauge coming off it — and because a `wait_timeout` refusal no longer implies a pool that is getting through its work: where the pool frees no worker for the whole of a request's wait, that counter moves too, and the remedy is the opposite one — shorten the wait budget rather than lengthen the pool. The server also says so in its log, which is where an operator already looking at the incident will be: a `WARN` on the scan that starts shedding, repeated about once a minute while it lasts, and an `INFO` once a whole minute has passed without a refusal. See [Prometheus Metrics](metrics.md).
