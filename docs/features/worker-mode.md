...
- **Superglobals** — `$_GET`, `$_POST`, `$_SERVER`, `$_COOKIE`, `$_FILES`, `$_REQUEST`, and `php://input` are repopulated with the new request data
...
What none of this does is diagnose a worker that has wedged rather than failed: a request stuck in a syscall is reported through `oxphp_worker_stuck_total` for an operator to act on, not cancelled and not counted. Nothing in the server ends such a request automatically — no recycle, no cancellation — so a wedged worker stays wedged until the call returns or the process is restarted. `max_execution_time` is not a backstop for it either, for the reasons in [Request Deadlines](#request-deadlines).
...
`max_execution_time` bounds a worker-mode request reliably only while that request is the only one its worker is carrying. The limit is a timer of the worker thread, not of each request, and with [fiber multiplexing](fiber-multiplexing.md) several requests share it:

- **A suspended request is not ended by its deadline.** A request waiting in `oxphp_async_await()`, `oxphp_sleep()`/`oxphp_usleep()`, or a `sleep()` or socket read taken over by `RUNTIME_HOOKS` runs no PHP code, and the timer can only end code that is running. The request resumes when what it waits for arrives — the promise settles, the sleep elapses, the socket becomes readable or reaches the stream's own timeout (`default_socket_timeout` unless `stream_set_timeout()` set another) — or when a server shutdown passes its drain deadline.
- **A request taken while others are in flight gets no deadline of its own.** The timer is started when the worker picks up a request with nothing else running on it. A request taken while another is suspended runs on whatever is left of that timer, and once the timer has fired it is not started again until the worker next has nothing in flight. A `set_time_limit()` call does restart it, but for the worker rather than for the caller: every request on that worker then runs against the new limit, counted from the call. On a worker that always has a request suspended — one serving long-lived streams, for example — the limit stops applying.
- **When the timer fires, it ends the request that happens to be running** — not necessarily the one that has been alive longest — and that request is read as having run out of time: a `504`, neutral for the recycle count.

Nothing else ends such a request for you. A client disconnecting does not end a non-streaming request, which runs to its end by design (see [Recycling](#recycling)), and a request running longer than the supervisor's threshold is reported through `oxphp_worker_stuck_total`, not cancelled. Put the limit on the wait itself: pass a timeout to `oxphp_async_await()` and its variants, and set timeouts on sockets and database clients.
