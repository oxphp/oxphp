...
- **Consecutive errors** — the worker took 3 consecutive requests that came apart: a fatal error, an out-of-memory, a stack overflow. Three in a row say the worker rather than any one request is at fault, and a worker that keeps coming apart is replaced rather than patched up again — see below for what does and does not count
