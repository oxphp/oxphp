...
5. Each fiber's PHP state — `$_GET`, `$_POST`, `$_COOKIE`, `$_SERVER`, `$_FILES`, `$_REQUEST`, response headers, output buffers, VM stack — is saved on suspension and restored on resumption, so a resuming fiber finds its own rather than whatever ran in the window. What is not on that list is not parked per fiber: while two requests overlap on a thread, they share it. The session is the one to know by name — `$_SESSION`, the session id and the open session behind them are one set per worker thread, so a request the scheduler admits beside a suspended one that left a session open is handed that session rather than its own, and `session_write_close()` does not separate them; see [Worker Mode](worker-mode.md#what-gets-reset-between-requests). `$_ENV` goes further — it is worker state rather than request state by design, so a write one request makes there is read by every other request that worker serves, overlapping or not — see [Superglobals](../php/superglobals.md#_env)
