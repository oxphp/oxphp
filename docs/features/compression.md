...
1. **Accept-Encoding check** — the client's `Accept-Encoding` header is parsed for `br` (Brotli) support, following RFC 9110 §12.5.3. Coding names are matched case-insensitively; `br;q=0` is read as a refusal rather than as support; and a `*` covers Brotli when the header does not name it explicitly. Requests whose header neither accepts `br` nor carries a `*` are never compressed.
...
A static file small enough to sit in the content cache (1 MiB or less) is compressed once rather than on every request. Once such a file has been served twice, OxPHP compresses it at maximum Brotli quality on a background thread and keeps the result next to the cached bytes; every later request for it is answered from that stored copy.

This is invisible from the outside apart from the response getting smaller — maximum quality typically produces 12–19% less than the per-request level. No request waits for the compression: the one that triggers it, and any that arrive while it runs, are served at the configured `COMPRESSION_LEVEL` as before. Response headers do not change.

The stored copy shares the cached file's validator, so it is discarded together with the cached bytes when [`STATIC_REVALIDATE`](static-files.md) notices the file changed on disk, and it counts against the same content-cache budget. Files that do not compress are marked and not retried. Setting `COMPRESSION_LEVEL=0` disables this along with all other compression.
...
- The client does not advertise `br` in the `Accept-Encoding` header, or advertises it with a zero weight (`br;q=0`)
...
Verify that the client sends `Accept-Encoding: br`. Most modern browsers do, but some HTTP testing tools do not include it by default. Note that Chromium-based browsers advertise `br` only over HTTPS, so a site served over plain HTTP receives uncompressed responses from them.

A weight of zero is a refusal, not a preference: `Accept-Encoding: br;q=0` disables compression for that request as surely as omitting `br` entirely.
