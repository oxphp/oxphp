...
|--------------------|--------|------------------|
| `exception` | A `#[OxPHP\Apm\Trace]` function that threw, `oxphp_apm_error()`, or an unhandled exception / fatal on the request's root span (see [Automatic root-span exception capture](#automatic-root-span-exception-capture)) | `exception.type`, `exception.message`, `exception.stacktrace` |
...
### Automatic root-span exception capture

When a request fails with an **unhandled exception** or a **fatal error** and returns a 5xx, OxPHP attaches an `exception` event to the request's **root span** automatically — no `#[OxPHP\Apm\Trace]` attribute and no `oxphp_apm_error()` call required. This makes a 500 self-describing in the trace and populates backends that group errors by the exception event (e.g. New Relic Errors Inbox).

The event carries the standard `exception.type`, `exception.message`, and `exception.stacktrace`, plus two OxPHP extensions, `exception.file` and `exception.line`, pointing at the throw site (or the fatal's location). For a **classless fatal** — a `trigger_error(…, E_USER_ERROR)`, an out-of-memory abort, or an execution-timeout — `exception.type` is a synthetic name (the PHP error constant, e.g. `E_USER_ERROR`) and no stacktrace is present. (Calling an undefined function is *not* a classless fatal on PHP 8: it throws a normal `Error`, which carries a full stacktrace like any other exception.) Message and stacktrace obey the same `OTEL_APM_MESSAGE_MAX_BYTES` / `OTEL_APM_STACKTRACE_MAX_BYTES` caps as the other exception events.

This works for raw PHP, applications without a custom exception handler, and worker-mode handlers.

**Boundary — frameworks that swallow exceptions (traditional request path).** In Traditional / Framework / SPA modes, if your application installs `set_exception_handler()` and renders its own error page (Laravel, Symfony, WordPress, …), the exception is *handled* from the engine's point of view — it never propagates uncaught, so OxPHP sees only the 500 status and cannot recover the Throwable. Automatic capture does not fire for those requests; record the exception explicitly from your framework's error reporter with `oxphp_apm_error($e)` instead.
...
**Streaming responses.** For a streamed response (SSE, or any `oxphp_stream_flush()` loop), the HTTP status is fixed once the headers are on the wire, and the request is treated as complete at that point. A fatal thrown *after* the status has been committed — on a streaming response, or on one that called `finish_request()` — is logged only; it is **not** added to the root span (the trace still shows the span, without the `exception` event). This is a documented boundary and holds equally for a committed **5xx** and a committed **2xx**.
