...
The cookie is not stripped from the request: the token stays visible to the
code being profiled and to anything that logs or forwards cookies. It appears
in `$_COOKIE` like any other cookie when the browser sends its cookies on a
single `Cookie` header, and in `$_SERVER['HTTP_COOKIE']` when it splits them
across several, which HTTP/2 permits. One more reason to keep a production
token off this path — see §17.
...
| `PROFILER_MAX_SPANS` | `50000` | Hard cap on the calls observed per request. Once it is reached no further spans are recorded, and the run is flagged `truncated`. With no depth limit set this is also the size of the tree; under `PROFILER_MAX_DEPTH` it is not, because a call the depth limit discards is counted here before the limit is applied — a truncated run then reaches disk with a fraction of this many spans. |
| `PROFILER_MAX_DEPTH` | `0` (no cap) | Maximum nesting depth of recorded calls. A call nested deeper is not recorded, and normally neither is anything it calls — that subtree is absent from the profile rather than flattened into it. Its time is not: the call at the limit still spans everything it did, and with nothing recorded underneath it that duration reads as its own exclusive time, so a frame at the boundary looks hot in itself rather than hot in what it called. The exception to the depth rule is a function that recurses into itself more than 32 frames deep: past that the engine observer loses track of which call a return belongs to and attributes it to a shallower frame of the same function, so a call deeper than the limit can still be recorded and the durations around it should not be trusted. The effective maximum is `32`, the depth to which the engine observer tracks open calls; a larger value is accepted and clamped, and — with `PROFILER_ENABLED=true` — reported as clamped at startup. This bounds the profile, not the cost of producing it: the engine hook still runs on the deeper frames, and the calls it drops are still counted against `PROFILER_MAX_SPANS`. Spans an application pushes through the APM helpers are not subject to it. |
...
| `PROFILER_EXPORT_XHGUI` | `auto` | Force xhgui envelope mode. Auto: URL path ends with `/run/import` (the canonical xhgui endpoint; host/query hints are not matched — set this to `true` for a non-standard path). |
| `PROFILER_EXPORT_BUGGREGATOR` | `auto` | Force the Buggregator envelope. Auto: URL path ends with `/api/profiler/store`. The envelope always emits xhprof, so `PROFILER_EXPORT_FORMAT` is ignored for it (a non-xhprof value is warned, not fatal). Mutually exclusive with `PROFILER_EXPORT_XHGUI` (enabling both is a startup error). |
| `PROFILER_EXPORT_APP_NAME` | *(unset)* | Buggregator `app_name` — the project a profile is grouped under. |
| `PROFILER_EXPORT_TAGS` | *(unset)* | Buggregator `tags`, a `key=value,key2=value2` list for filtering. A malformed token (not `key=value`), an empty key, or a duplicate key is a startup error. |
...
### 10.5. Buggregator (local debug server)

[Buggregator](https://buggregator.dev) is a single-binary debug server that,
among other things, renders xhprof profiles as flame graphs grouped by project.
The `xhprof` push targets its `POST /api/profiler/store` endpoint directly — no
xhprof PHP extension or client library needed, OxPHP's native profiler produces
the data.
...
services:
  ...
  app:
    ...
    environment:
      ...
      PROFILER_EXPORT_TAGS: "env=staging,region=eu" # filterable in the UI
```

A URL whose path ends in `/api/profiler/store` auto-selects the Buggregator
envelope (`PROFILER_EXPORT_BUGGREGATOR: "true"` forces it for a custom URL;
`"false"` opts out). This envelope always emits xhprof, so `PROFILER_EXPORT_FORMAT`
is ignored for it (a non-xhprof value is warned at startup, not fatal — the
profiler never crashes the server over an export knob). `app_name` and `tags`
drive Buggregator's project grouping and filtering; without them the profile
still renders but lands ungrouped. `hostname` comes from `$HOSTNAME`, falling
back to the `gethostname(2)` syscall when that variable is not set.
...
- xhgui envelope auto-detection: URL **path ends with `/run/import`** (the
  canonical xhgui endpoint). A fuzzy `xhgui` substring in the host or query is
  not matched — force via `PROFILER_EXPORT_XHGUI=true|false` for such URLs.
...
- Reasonable `PROFILER_MAX_SPANS` — once it is reached no further spans
  are recorded, the request itself is not interrupted, and the prefix
  collected up to that point is written with `truncated: true`. For very
  large requests prefer a surgical `start()`/`stop()` around the section
  of interest.
