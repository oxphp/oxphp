...
- **`oxphp_async_await_all()` and `oxphp_async_await_any()` do not yield** — they currently block even inside a fiber. Use sequential `oxphp_async_await()` calls for fiber-friendly behavior
