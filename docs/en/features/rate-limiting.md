...
| `RATE_WINDOW` | Window duration in seconds | `60` |
...
A background Tokio task runs every 60 seconds and removes entries whose window expired more than `2 * RATE_WINDOW` seconds ago. This prevents the `DashMap` from growing indefinitely when clients send a burst of requests and then disappear.
