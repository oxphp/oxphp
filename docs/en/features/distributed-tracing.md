...
The event carries the standard `exception.type`, `exception.message`, and `exception.stacktrace`, plus two OxPHP extensions, `exception.file` and `exception.line`, pointing at the throw site (or the fatal's location). For a classless fatal (e.g. a call to an undefined function) `exception.type` is a synthetic name and no stacktrace is present. Message and stacktrace obey the same `OTEL_APM_MESSAGE_MAX_BYTES` / `OTEL_APM_STACKTRACE_MAX_BYTES` caps as the other exception events.

This works for raw PHP, applications without a custom exception handler, and worker-mode handlers.

**Boundary — frameworks that swallow exceptions.** If your application installs `set_exception_handler()` and renders its own error page (Laravel, Symfony, WordPress, …), the exception is *handled* from the engine's point of view — it never propagates uncaught, so OxPHP sees only the 500 status and cannot recover the Throwable. Automatic capture does not fire for those requests; record the exception explicitly from your framework's error reporter with `oxphp_apm_error($e)` instead.
