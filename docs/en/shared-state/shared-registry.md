...
Per-entry detail — id, type, refcount, byte cost — lives on the [internal introspection endpoint](../operations/internal-server.md) at `/__ox_shared/entries`. There is intentionally no per-entry PHP API to avoid duplicating that surface.
