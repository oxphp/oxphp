...
Для вынікаў `Serve`, `static_file::serve()` чытае файл з дыска (з файлавым кэшам для метададзеных), вызначае MIME-тып і вяртае адказ з адпаведнымі загалоўкамі `Content-Type` і `Content-Length`.
...
| 100 | `ServerHeaderHandler` | Дадае загалоўкі `Server: OxPHP/{version}` і `X-Request-ID` |
