...
| `0` або не ўсталяваны | Статычны | `PHP_WORKERS=0` | Фіксаваны пул з CPU / 2 воркераў (мінімум 1) |
...
| `0:0` | Дынамічны | `PHP_WORKERS=0:0` | MIN аўтаматычна (CPU/4, мін. 1), MAX аўтаматычна (CPU * 2) |
...
- Воркер быў бяздзейным даўжэй за `PHP_WORKERS_IDLE_SECONDS` (па змаўчанні 30 с)
...
|---|---|---|
| `PHP_WORKERS` | `0` (CPU / 2, мін. 1, статычны) | Рэжым пула воркераў. `N` для статычнага, `MIN:MAX` для дынамічнага |
| `PHP_WORKERS_IDLE_SECONDS` | `30` | Таймаўт бяздзейнасці перад спыненнем дынамічнага воркера |
| `QUEUE_CAPACITY` | `PHP_WORKERS * 128` | Ёмістасць абмежаванага канала (выкарыстоўвае пачатковую колькасць для дынамічнага) |

## Рэжым воркера (Persistent PHP)

Рэжым воркера — гэта альтэрнатыўная мадэль выканання, пры якой PHP-працэсы застаюцца жывымі паміж запытамі, пазбягаючы накладных выдаткаў `php_request_startup()` / `php_request_shutdown()` для кожнага запыту. Уключаецца ўстаноўкай зменнай асяроддзя `WORKER_FILE`.

### Як гэта працуе

Замест стандартнага жыццёвага цыклу кожнага запыту (startup → execute → shutdown), рэжым воркера выконвае адзіны PHP-скрыпт, які выклікае `oxphp_worker()` з callback-апрацоўшчыкам. Апрацоўшчык выклікаецца для кожнага запыту, а паміж запытамі **мяккі скід** ачышчае стан кожнага запыту без знішчэння PHP-кучы:

```
Жыццёвы цыкл патоку воркера:

  php_request_startup()           <- выконваецца АДЗІН РАЗ
  require worker.php              <- загрузка: autoload, злучэнне з БД, канфіг
  oxphp_worker(function() {       <- уваход у цыкл воркера
      ┌─────────────────────────┐
      │ чаканне запыту          │ <- блакуецца на crossbeam channel
      │ мяккі скід              │ <- перазапаўненне суперглабалаў, ачыстка вываду
      │ выклік handler()        │ <- выкананне карыстальніцкага коду
      │ адпраўка адказу         │ <- адказ на ўзровень HTTP
      │ праверка лімітаў        │ <- max_requests, max_memory
      └─────────────────────────┘
      │ вяртанне ў цыкл ↑      │
  })
  php_request_shutdown()          <- выконваецца АДЗІН РАЗ (пры выхадзе)
```

Мяккі скід паміж запытамі:
- Перазапаўняе `$_GET`, `$_POST`, `$_SERVER`, `$_COOKIE`, `$_FILES` з новага запыту
- Ачышчае і скідвае буферы вываду
- Скідвае загалоўкі і код стану HTTP-адказу
- Выклікае і ачышчае апрацоўшчыкі `register_shutdown_function()`

### Рэцыклінг

Воркеры рэцыклуюцца (завяршаюцца і перазапускаюцца) на аснове наладжвальных лімітаў:

| Прычына выхаду | Трыгер | Пазнака метрыкі |
|---|---|---|
| `max_requests` | Дасягнуты `WORKER_MAX_REQUESTS` | `reason="max_requests"` |
| `max_memory` | Перавышаны `WORKER_MAX_MEMORY_MIB` (MiB) | `reason="max_memory"` |
| `error` | Неперахопленае выключэнне або фатальная памылка ў апрацоўшчыку | `reason="error"` |
| `shutdown` | Плаўная спынка сервера | *(не лічыцца рэцыклінгам)* |

Калі воркер завяршаецца па нешатдаўн-прычыне, манітор здароўя (статычны рэжым) або ScaleManager (дынамічны рэжым) перазапускае яго аўтаматычна. Новы воркер паўторна выконвае ўвесь скрыпт воркера, уключаючы код загрузкі.

### Метрыкі

Рэжым воркера прадастаўляе спецыяльныя метрыкі Prometheus для маніторынгу здароўя персістэнтных воркераў:

- **`oxphp_worker_requests_handled_total`** — агульная колькасць запытаў, апрацаваных усімі воркерамі
- **`oxphp_worker_recycles_total`** / **`oxphp_worker_recycles_by_reason_total`** — колькасць рэцыклінгаў, глабальна і па прычыне
- **`oxphp_worker_memory_bytes{worker="N"}`** — бягучае выкарыстанне PHP-кучы кожным воркерам
- **`oxphp_worker_uptime_seconds{worker="N"}`** — час з моманту запуску кожнага воркера
- **`oxphp_worker_request_duration_us`** — гістаграма часу выканання PHP-апрацоўшчыка (без уліку чакання ў чарзе)

Глядзіце [Метрыкі](../operations/metrics.md#рэжым-воркера) для поўнай даведкі і прыкладаў запытаў PromQL.

### Канфігурацыя

| Пераменная | Па змаўчанні | Апісанне |
|---|---|---|
| `WORKER_FILE` | *(няма)* | Шлях да PHP-скрыпту воркера (адносна `DOCUMENT_ROOT`). Уключае рэжым воркера, калі ўсталяваны |
| `WORKER_MAX_REQUESTS` | `0` | Максімальная колькасць запытаў перад рэцыклінгам. `0` = без ліміту |
| `WORKER_MAX_MEMORY_MIB` | `0` | Максімальная памяць (MiB) перад рэцыклінгам. `0` = без ліміту |

### Інтэграцыя з маршрутызацыяй

Калі ўсталяваны `WORKER_FILE`, маршрутызацыя змяняецца: запыты, якія не з'яўляюцца статычнымі файламі і не адпавядаюць файлу на дыску, маршрутызуюцца на скрыпт воркера замест вяртання 404. Статычныя файлы (CSS, JS, выявы) па-ранейшаму абслугоўваюцца напрамую з дыска. Гэта падобна на дырэктыву `try_files` у nginx.
