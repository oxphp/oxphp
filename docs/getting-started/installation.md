...
OxPHP is distributed as a Docker image — the fastest and recommended way to start serving PHP applications. The image bundles the server binary, PHP 8.4 or 8.5 ZTS, the OxPHP extension, and all runtime dependencies on Alpine Linux. The default `:0.12.0` and `:latest` tags ship PHP 8.5; pull PHP 8.4 with the `:0.12.0-php8.4`, `:php8.4`, or any `*-php8.4*` tag variant.
...
```bash
docker pull ghcr.io/oxphp/oxphp:0.12.0
```
...
│       └── sodium.so, ...                           # from the PHP base image; the set depends on the tag
└── etc/php/
    └── conf.d/
        ├── docker-php-ext-*.ini                     # from the PHP base image; the set depends on the tag
...
The image carries no `php.ini` of its own — beside `conf.d/`, `/usr/local/etc/php/` holds only the upstream `php.ini-development` and `php.ini-production` samples, and PHP loads neither — and no application settings either. Supply yours as a `.ini` file under `conf.d`, mounted or copied in; see the [Docker Guide](docker.md). PHP parses every `.ini` file in that directory in alphabetical order and skips any other name without a word, so what you call the file decides both whether it is read at all and where it lands relative to the image's own. For an ordinary setting the last value read is the one that applies, so a file that overrides one has to sort after the file that sets it; `extension=` and `zend_extension=` are collected from every file instead and all loaded, so a later `extension=` never replaces an earlier one.

The three OxPHP components and their purpose, with the sizes measured in the `0.11.0` image — they differ between `linux/amd64` and `linux/arm64` and move from release to release:

| Component | Size | Purpose |
|-----------|------|---------|
| `oxphp` | ~12–15 MB | HTTP server, routing, plugins, metrics |
| `liboxphp_bridge.so` | ~150–170 KB | Shared bridge library that links the server to the PHP runtime |
| `oxphp_sapi.so` | ~700 KB | PHP functions (`oxphp_request_id()`, `OxPHP\Http\Request`, etc.) |
...
COPY --from=ghcr.io/oxphp/oxphp:0.12.0 /usr/local/bin/oxphp /usr/local/bin/oxphp
COPY --from=ghcr.io/oxphp/oxphp:0.12.0 /usr/local/lib/liboxphp_bridge.so /usr/local/lib/
COPY --from=ghcr.io/oxphp/oxphp:0.12.0 /usr/local/lib/php/extensions/no-debug-zts-20240924/oxphp_sapi.so /usr/local/lib/php/extensions/no-debug-zts-20240924/
