...
**返回值：** 当 OxPHP 生成 ID 时，返回 16 位十六进制字符串（例如 `"67b9a3c11a2b0042"`）。当客户端发送 `X-Request-ID` 请求头时，直接返回该值（1–64 个字符，由字母数字及 `-`、`_`、`.` 组成）。
...
**参数：**
- `$handler` — 每次请求调用一次。处理程序不接收参数；使用超全局变量（`$_SERVER`、`$_GET`、`$_POST` 等）获取请求数据。
