...
- **PHP 执行拒绝名单** — 在直接映射模式（传统模式和 SPA 模式）下，`PHP_DENY_PATHS` 在任何磁盘 I/O 之前阻止匹配指定 glob 模式（如 `/uploads/**`，或单个文件 `/admin/legacy.php`）的 `.php` 执行。参见 [PHP 执行拒绝名单](../security/php-deny.md)
