...
4. **请求之间**，超全局变量（`$_GET`、`$_POST`、`$_SERVER`、`$_COOKIE`、`$_FILES`、`$_REQUEST`、`php://input`）、输出缓冲区和响应头会自动重置。软重置会清理每次请求的状态，同时保留外部作用域中已引导的资源。`$_ENV` 是例外，它被刻意**不**重置，参见[超全局变量](../php/superglobals.md#_env)。
