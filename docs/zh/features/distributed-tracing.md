...
**流式响应。** 对于流式响应（SSE 或任何 `oxphp_stream_flush()` 循环），一旦响应头发出，HTTP 状态就已固定，此时请求即视为已完成。在状态提交*之后*抛出的致命错误——无论是流式响应，还是调用了 `finish_request()` 的响应——只会被记录；它**不会**被添加到根 span（trace 中仍显示该 span，只是没有 `exception` 事件）。这是一条明确的边界，对已提交的 **5xx** 和已提交的 **2xx** 同样适用。
