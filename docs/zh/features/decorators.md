...
在 Worker 模式下，PHP 进程在多个请求间持久化，但**装饰器实例不会**：每个 Worker 的实例缓存在每个请求结束时（RSHUTDOWN）被清空。这意味着：

- 构造函数逻辑每个请求每个被装饰函数运行一次——在该函数于该请求内的首次调用时——而非在 Worker 的整个生命周期内只运行一次
- 属性中的实例状态会在单个请求内被对被装饰函数的每次调用共享，但**不会**延续到下一个请求
- 每个请求都以全新的实例开始（并伴随一次全新的构造函数运行，重新评估属性参数）
...
属性参数是源代码层面的字面量，在装饰器实例被构造时（每个请求的首次调用时）评估。它们不是运行时值，因此变动的应用数据无法改变它们——只有当你在代码中编辑属性时它们才会改变。

**修复：** 使用 `before()` 进行请求级状态处理，而非构造函数。构造函数只应接受属性中声明的静态配置。
