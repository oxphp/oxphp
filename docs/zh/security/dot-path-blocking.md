...
单层百分号编码的绕过尝试会被捕获——`/%2egit/HEAD` 一次解码后变为 `/.git/HEAD` 并被拒绝。双重编码的输入（`/%252egit/HEAD`）只解码一次，仍保持为 `/%2egit/HEAD`，本层不会将其识别为点段。标准 HTTP 客户端和浏览器不会双重编码，因此实际中这种情况很少出现；如果你的上游代理在转发前会规范化 URI，则单次解码已经足够。
