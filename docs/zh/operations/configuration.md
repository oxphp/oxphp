...
唯一一处钩子明显比原生更贵的场景是宽泛的 `stream_select()`：调用中列出的每个描述符都要在等待前注册、等待后移除。实测每次调用：单个描述符约 6 微秒对无钩子的 5 微秒，64 个时 56 对 9 微秒，200 个时 130–150 对 16–21 微秒——大约每个描述符 0.65 微秒。换来的是 Worker 线程本身，而无钩子的调用会在整个等待期间占住它。只要 Worker 还有别的请求要服务，这笔交易就划算；而当请求本身就是事件循环、线程再无他事可做时则不划算——对这种宽轮询循环，不要启用 `streams`。
