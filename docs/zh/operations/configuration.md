...
| `MAX_CONNECTIONS` | `10000` | 最大并发 TCP 连接数。同时也是默认 `QUEUE_MAX_WAITING` 的上限（其一半），因此取值格式错误会导致启动失败，而非静默回退到默认值 |
...
| `QUEUE_CAPACITY` | 初始工作进程数 × 128 | PHP 队列中最大待处理请求数。队列已满时到达的请求会等待空闲槽位（参见 `QUEUE_WAIT_TIMEOUT_MS`）。对于动态进程池（`MIN:MAX`），初始工作进程数等于最小值。`0` = 自动 |
| `QUEUE_WAIT_TIMEOUT_MS` | `1000` | 请求等待 PHP 工作进程的最长毫秒数，超时后以 529 拒绝。这是在请求到达时打上的单一截止时间，涵盖请求可能经历的两段等待：等待队列槽位，以及入队之后等待工作进程。若工作进程取到该请求时截止时间已过，则在取出时直接拒绝而不再执行——因此该预算限制的是整段等待，而不只是入队这一半；它并不限制处理器随后的执行时长。同时等待的请求最多为 `QUEUE_MAX_WAITING` 个，超出后立即拒绝。`0` = 队列满时一律立即拒绝。若应用会通过 HTTP 回调本服务器，应调小该值或设为 `0` —— 外层请求释放工作进程之前内层请求无法入队，等待纯属浪费；若前置负载均衡器自身的超时更短，同样应调小。等待期间离开的客户端会立即让出自己的位置，因此按超时重试的负载均衡器不会用自己已放弃的尝试填满等待集合——但这仅限于断开能够被感知的场景。HTTP/1.1 客户端在请求中途关闭连接时，服务器在下一次写入之前无从察觉，此时被放弃的尝试确实会占据等待位直到预算耗尽；将预算设置得低于均衡器的超时即可避免这种情况 |
| `QUEUE_MAX_WAITING` | 初始工作进程数 × 128，上限为 `MAX_CONNECTIONS` / 2 | 同时等待队列槽位的最大请求数。超出后请求将被立即拒绝而不再等待。每个等待者在整个等待期间都占用一个连接和一份完整缓冲的请求体，因此该值限制的是被占用的资源，而非「值得等待」的数量；`MAX_CONNECTIONS` / 2 的上限为服务器保留余量，使其在持续过载时仍能接受连接并给出拒绝。请根据自身处理器的延迟来设定——见下文。`0` = 自动，且不低于 1 |

### 等待集合的容量设定

真正*有意义*的等待数量取决于进程池消化请求的速度。设有 `W` 个工作进程、处理器耗时 `T` 毫秒，则进程池每毫秒可接纳 `W / T` 个请求，因此 `B` 毫秒的预算大约只能放行 `W × B / T` 个等待者。超出的部分会等满整个预算并最终仍被拒绝，其间始终占用一个连接和一份缓冲的请求体。

默认值无法计算这一点——启动时并不知道服务速率——因此它刻意取得宽松。这适合快速处理器：进程池能在预算内轻松消化很深的积压；对慢处理器则过大：8 个工作进程、默认 1 秒预算、200 毫秒的处理器，来得及接纳的约为 40 个请求，而默认最多会驻留 1024 个。

```bash
QUEUE_MAX_WAITING=40
```

将其设为接近 `W × B / T` 的值，可把多余部分变成立即返回的 529，而不是一秒之后的 529。调小 `QUEUE_WAIT_TIMEOUT_MS` 可从另一项取得相同效果——两个参数彼此制衡，而在慢处理器上通常更适合缩短预算：它同时也限制了客户端等待被拒绝的时长。
...
- **写入。** 只有读取会等待。只要描述符由单个 fiber 拥有、没有别的代码从中取走数据，读就绪就保持有效；而发送缓冲区的空间由对端授予和收回，因此因可写而被唤醒的 fiber，在真正写入时可能发现窗口已经关闭，此后 PHP 无论如何都会为其完整超时阻塞线程。所以填满套接字缓冲区的写入，其行为与不启用钩子时完全相同。实践中代价很小：耗时的是等待应答，而不是把查询交给内核。（写入仍会被检查，但只为了一件事：这条连接当前承载的是哪个 fiber 的交互——见下文关于共享连接的说明。在没有别的 fiber 使用的连接上，也就是通常情形，写入原封不动地走原生路径。）
...
`stream_select()` 的钩法是：先等待三个数组所指的描述符就绪，然后把调用交给 PHP 本身，并将超时置零——因此所有可观察的部分仍由 PHP 决定：返回的计数、把数组重写为就绪流、警告以及参数错误。当数组中出现钩子不愿代劳的内容时，等待会被跳过，调用与没有钩子时完全一致：已缓冲数据的读取流（`stream_select()` 直接从缓冲区作答，根本不看描述符）、完全没有描述符的流、已不是有效流的元素（例如遗留在数组里的已关闭流——PHP 对它的回应是报错而非等待）、内核根本不为其跟踪就绪状态的描述符（普通文件是典型情形，而它在被问及的那一刻就算作就绪），或取值达到或超过 `FD_SETSIZE` 的描述符——PHP 自己的 `select()` 会直接拒绝它。最后一条是真实的上限而非形式：繁忙的 Worker 完全可能持有超过 1024 个打开的描述符，而指向这类描述符的 `stream_select()` 无论有无钩子都以同样的方式失败。

另有两点值得了解：

- **在并发 fiber 之间共享同一个连接，对 OxPHP 所守护的客户端而言是安全的，但不会带来任何收益。** 这并非边缘情形，而是 worker 模式下应用的常态：WordPress、Laravel 和 Symfony 都在 worker 启动时建立数据库与缓存客户端，并把同一个交给每个请求；除非重写数据访问层，否则无法要求它们为每个 fiber 各开一个连接。客户端协议是一连串的交换——写出命令、读回应答——而连接上没有任何标记指出一次交换在哪里结束，因此挂起在读取上的 fiber 正处在一次交换的中间，第二个 fiber 的命令落在那里就会破坏协议。两个客户端在这一点上的失败方式不同，且都不好：mysqlnd 跟踪自己的连接状态，会在发送任何字节之前就拒绝该命令；而 phpredis 没有这种检查，两个 fiber 会读到彼此的应答——一个请求的数据被交给了另一个请求，且不报任何错误。因此 fiber 在使用连接之前先占用它，并且在必须发生的两个层面上都这样做：保持字节顺序的套接字操作，以及 PDO 与 mysqli 的客户端入口点——因为 mysqlnd 的拒绝发生在任何 I/O 之前，套接字层面的守卫根本来不及介入。另一个 fiber 到达同一连接时，会在客户端层面等待它被释放——在那里除了连接的标识之外并不持有任何东西；而在套接字层面它不会等待，而是被拒绝，与套接字超时被拒绝的方式相同，因为挂起在别人的流操作内部的 fiber 会持有一个其拥有者随时可能释放的指针。phpredis 也正因如此在客户端层面逐个方法地受到保护。客户端层面占用的是连接本身，而不是持有它的 PHP 对象，所以通过多个 PDO 对象访问的持久连接算作同一个连接，而用 `PDO::connect()` 打开的连接（它返回驱动自己的子类而非 `PDO`）与其他连接一样受到保护。因此，这个钩子的收益应理解为属于 fiber 自己打开的连接：发起自己的 HTTP 或数据库调用的异步任务，或自行建立客户端的请求。共享连接得到的是另一样东西——等待期间归还工作线程，让其他请求得以执行与该连接无关的工作；而该连接上的交换本身依次进行，与关闭钩子时完全相同。有三条边界值得了解。**第一**，查询过一次的 fiber 会把连接持有到自己请求结束，因为请求结束是第一个必定越过交换结尾的时刻；在它自己等待别的事情时同样照持不放，因此一个已经查询过、随后又去等待自己发起的、需要同一连接的工作的请求，等的其实是自己——这样的一对会在下面那条界限上散开，而不是继续下去。**第二**，等待始终有界：以 `max_execution_time` 与 `default_socket_timeout` 中较小者为界。两者都不设置时该界限是 30 秒——服务器 SAPI 取引擎默认值，前者 30 秒、后者 60 秒；只有在 `max_execution_time` 为 0 的部署里，界限才是 `default_socket_timeout` 的 60 秒。`max_execution_time` 取请求当前的值——`set_time_limit()` 正是请求声明自己可运行多久的方式；`default_socket_timeout` 则取进程启动时的值，因为它是套接字*操作*的默认期限，而某个请求为自己的调用把它调小（库的常见做法，而且往往不再还原）不应缩短同一 worker 上后续请求的这条界限。越过该界限后，调用退回到无保护的行为，并在服务器日志中写明原因：PDO 与 mysqli 交给客户端自己作答——客户端对交换中途发出的命令本就会拒绝，那正是应用已经在处理的错误；而 phpredis 没有这种拒绝，交给它只会读到别人的应答，因此改为抛出 `RedisException`，一个字节也不发出。套接字层面的冲突没有自己的界限，因为那里根本不等待：操作立即失败，与超时的失败方式相同，`stream_get_meta_data()` 报告 `timed_out`。因此，两个各自持有对方所等待之物的 fiber 会在该界限上散开，而不是永远互相等待。**第三**，有若干情况刻意不在覆盖范围内。跨请求保留的 statement 或结果对象，其前面没有任何占用连接的调用：对上一个请求中准备的 statement 调用 `PDOStatement::execute()`，其行为与完全没有占用机制时相同。在另一个 fiber 正处于交换中途时，在持久连接上构造第二个句柄：PDO 在交出池中连接之前会检查它是否存活，交换中途这项检查不会通过，而 PDO 对此的回应是丢弃该连接。在裸套接字上手写的协议——写出命令、因别的原因挂起、稍后再读应答：套接字层面只在持有者挂起等待该应答期间才拒绝，而在这两点之间连接会被另一个 fiber 接管；上面三个客户端在此处由客户端层面的占用兜底，手写协议没有对应物。以及完全在 fiber 之外触及连接的一切，例如引擎的循环回收器在请求之间运行的析构函数——任何占用都看不见它。
...
实测开销：在 Worker 上没有其他 fiber 挂起时，每次套接字往返约 3–5 微秒；有 64 个 fiber 挂起在描述符上时约 5–6 微秒，有 200 个时约 7–11 微秒。就绪状态通过内核在两次等待之间保留的集合来解析，因此该数字反映的是有多少描述符变为就绪，而不是有多少在等待。空闲的 Worker 会在这些描述符上等待，而不是先睡固定间隔、再到下一个 tick 才发现就绪——这一点至关重要：盲目等待的代价约为每次往返 2 毫秒。

宽泛的 `stream_select()` 带有窄调用所没有的开销：调用中列出的每个描述符都要在等待前注册、等待后移除。测量时刻意剔除了等待本身——所有描述符均已就绪，调用立即返回，剩下的只有开销：单个描述符约 6 微秒对无钩子的 5 微秒，64 个时 56 对 9 微秒，200 个时 130–150 对 16–21 微秒，即大约每个描述符 0.65 微秒。

请把它理解为每次调用的固定成本，而不是同样工作变慢了。真正会等待的 `stream_select()`——select 循环正是为此而写——远远盖过它：等待一毫秒时，即便是 200 个描述符的数字也只占该次调用的约十分之一，而换来的是 Worker 线程本身，无钩子的调用会在整个等待期间占住它。有两种形态例外，对它们最好不要启用该类别：几乎从不等待的多描述符调用（没有线程时间可回收），以及请求本身就是事件循环的情形（线程反正无事可做）。在单个连接上等待的客户端——mysqlnd、phpredis、HTTP 流包装器——处于表格最上一行，那里的开销约为一微秒。
