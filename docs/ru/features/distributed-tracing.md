...
**Потоковые ответы.** Для потокового ответа (SSE или любой цикл `oxphp_stream_flush()`) HTTP-статус фиксируется, как только заголовки ушли на провод. Фатальная ошибка, брошенная *после* того, как ответ уже зафиксировал статус **5xx**, всё равно захватывается — завершение запроса откладывается до закрытия потока, и поздняя ошибка доходит до корневого спана. Но поток, уже отправивший **2xx**, нельзя задним числом переметить в 5xx: фатал на уже идущем 2xx-потоке логируется, но не добавляет событие `exception` на корневой спан (в трейсе остаётся сам спан, без события `exception`).
