...
description: Атомарный int64-аккумулятор, разделяемый между PHP-воркерами — lock-free инкремент/декремент, signed-сложение, batch-аккумуляция, оконный сброс.
---

# Shared\Counter

`OxPHP\Shared\Counter` — это общепроцессное атомарное 64-битное знаковое целое, специализированное под **аккумуляцию**: подсчёт событий, суммирование дельт, оконные итоги. Каждая операция lock-free и линеаризуема; два воркера, инкрементирующих конкурентно, никогда не теряют тик.

Для произвольного атомарного состояния — конечных автоматов, version stamps, CAS-циклов, bitflag-масок — используйте [`Shared\Atomic`](shared-atomic.md).

## Обзор

- **Атомарный int64.** Диапазон `−9_223_372_036_854_775_808 … 9_223_372_036_854_775_807`. Переполнение оборачивается.
- **Lock-free.** `inc` / `dec` / `add` компилируются в единственный `fetch_add`.
...
final class Counter implements Shareable
{
    ...
    public function reset(): int;                     // возвращает предыдущее, атомарно обнуляет
...
| `reset`          | предыдущее   | Закрытие окна: атомарно прочитать итог и обнулить.               |
...
> Нужны `compareAndSet`, `swap` или другие низкоуровневые атомарные операции для конечных автоматов или version stamps? Counter — domain-аккумулятор; берите [`Shared\Atomic`](shared-atomic.md).
...
- **`reset()` возвращает предыдущее значение и атомарно обнуляет.** Это паттерн snapshot-and-zero (`LongAdder::sumThenReset`). Метода `reset(int $newValue)` нет; чтобы стартовать с ненулевой базы, создайте новый `Counter(initial: …)` или используйте `Shared\Atomic::store`.
