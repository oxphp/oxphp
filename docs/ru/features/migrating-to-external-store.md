...
- `Shared\Once::init(callable)` запускает фабрику в процессе, когда побеждает. Во внешнем хранилище фабрика должна быть идемпотентной (два писателя могут оба её запустить, и побеждает только одно значение) или вам нужна обёртка leader-election.
