...
| `$traceId` | `string` | Текущий W3C trace ID. Пустая строка, когда распределённая трассировка не активна |
...
Если `before()` выбрасывает исключение, OxPHP фиксирует его и вызывает `after()` в обратном порядке для всех декораторов, которые уже успели завершить свой `before()`. Вызывающий код увидит исключение через обычный `try`/`catch`, а `after()` на декораторе, чей `before()` бросил — **не** вызывается.

> **Важное ограничение «остановки выполнения».** PHP-API `zend_observer_fcall_begin`, который OxPHP использует для запуска `before()`, не даёт возможности отменить сам вызов. Когда `before()` бросает исключение, тело декорированной функции может успеть выполнить несколько opcode до того, как VM размотает стек до ближайшего обработчика. **Не полагайтесь на `RejectedException`, чтобы пропустить побочные эффекты внутри тела функции.** Относитесь к отказу декоратора как к «вызывающий увидит исключение», а жёсткую авторизацию реализуйте внутри тела функции (или перед вызовом), а не в декораторе.

## Остановка выполнения

Декоратор может сообщить об отказе, выбросив `OxPHP\Decorator\RejectedException` из `before()`. Исключение доходит до вызывающего, но (см. выше) это не «вето до вызова»:
...
В режиме воркера PHP-процесс сохраняется между запросами, но **экземпляры декораторов — нет**: per-worker кэш экземпляров очищается в конце каждого запроса (RSHUTDOWN). Это означает:

- Логика конструктора выполняется один раз за запрос для каждой декорированной функции — при первом вызове этой функции в рамках запроса — а не один раз за время жизни воркера
- Состояние экземпляра в свойствах разделяется всеми вызовами декорированной функции в рамках одного запроса, но **не** переносится в следующий запрос
- Каждый запрос начинается со свежего экземпляра (и со свежего запуска конструктора, заново вычисляющего аргументы атрибута)
...
При включённом APM-плагине (`OTEL_APM_ENABLED=true`) OxPHP регистрирует встроенный декоратор для атрибута `#[OxPHP\Apm\Trace]`. Он автоматически создаёт спан при входе в функцию и закрывает его при выходе — ручные вызовы `oxphp_apm_start()` / `oxphp_apm_end()` не требуются.
...
- **Максимум 256 уровней вложенности** — стек контекста декоратора поддерживает до 256 уровней вложенных вызовов декорированных функций. При превышении вызов бросает `OxPHP\Decorator\StackOverflowException` вместо тихой коррупции контекста декоратора
...
Аргументы атрибута — это литералы уровня исходного кода, вычисляемые при конструировании экземпляра (первый вызов в каждом запросе); это не значения времени выполнения.

**Решение:** используйте `before()` для состояния, специфичного для запроса. Конструктор должен принимать только статическую конфигурацию из атрибута.
