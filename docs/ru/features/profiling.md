...
### 5.5. Исключение путей из выборки

`PROFILER_SAMPLE_RATE` сэмплирует случайную долю **всех** запросов — включая
служебный трафик самого фреймворка, который засоряет данные. Web Debug Toolbar
Symfony опрашивает `/_wdt/{token}` и ссылается на `/_profiler/{token}`; Laravel
Debugbar и Telescope ведут себя так же. Уберите их из выборки через
...
Glob-паттерны через запятую, синтаксис тот же, что у `PHP_DENY_PATHS`: `*` не
пересекает `/`, `**` пересекает, ведущий `/` опционален. Голый путь **и** его
поддерево покрываются, только если перечислить оба — `/_profiler/**` ловит
`/_profiler/x`, но не голый `/_profiler`, отсюда рецепт из двух паттернов выше.
Паттерны матчатся по пути запроса как есть — без percent-декодирования и
нормализации `..` — поэтому указывайте литеральный путь, который использует ваш фреймворк.

> **Исключение действует только на автоматический сэмплинг.** Запрос с явным
> триггером — заголовок `x-oxphp-profile`, cookie `OXPROF` или query-параметр
> `__oxprof` — профилируется **всегда**, даже на исключённом пути. Это позволяет
> осознанно профилировать сам `/_profiler`, держа его вне фоновой выборки.
...
| `PROFILER_EXCLUDE_PATHS` | *(нет)* | Glob-паттерны через запятую (синтаксис `PHP_DENY_PATHS`), исключаемые из `PROFILER_SAMPLE_RATE`. Явные триггеры всё равно профилируют их. Пример: `/_profiler,/_profiler/**,/_wdt/**`. |
