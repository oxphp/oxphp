...
Два потолка защищают от allocation-бомб из недоверенного ввода; оба
настраиваются через переменные окружения и бросают перехватываемое
исключение вместо аварийного завершения процесса:

- **Ёмкость** — `new Channel($capacity)` заранее выделяет массив слотов.
  Ёмкость, чей массив слотов превысил бы `SHARED_MAX_CHANNEL_BYTES`
  (по умолчанию 64 MiB, ~2M слотов), бросает `CapacityException`. Каналы
  обычно имеют глубину от десятков до нескольких тысяч, поэтому это
  срабатывает лишь на патологических значениях. (Неположительный
  `$capacity` — это отдельный `TypeException`.) Эффективный бюджет
  поднимается как минимум до одного слота, поэтому нулевой или слишком
  маленький `SHARED_MAX_CHANNEL_BYTES` никогда не отклонит минимальный
  канал ёмкостью 1 — он лишь ограничивает большие ёмкости.
- **На значение** — каждое отправляемое значение сериализуется; значение,
  чей сериализованный размер превышает `SHARED_MAX_VALUE_SIZE`
  (по умолчанию 1 MiB), бросает `ValueTooLargeException`. Это относится к
  `send`, `trySend`, `sendTimeout` и `sendMany` (который отклоняет весь
  пакет, ничего не отправляя, если хотя бы один элемент превышает лимит).
  Это тот же лимит на значение, что применяет `Shared\Map`.
