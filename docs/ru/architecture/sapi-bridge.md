...
### Функция диспетчеризации плагинов

Расширение также регистрирует `oxphp_plugin_dispatch` — универсальный обработчик для всех зарегистрированных функций плагинов. Когда PHP-скрипт вызывает функцию плагина (например, `oxphp_debug_info()`), движок Zend перенаправляет вызов в `oxphp_plugin_dispatch`, который:

1. Читает имя функции из `execute_data->func->common.function_name`
2. Собирает все аргументы в PHP-массив и кодирует их через `json_encode`
3. Вызывает `oxphp_bridge_dispatch(func_name, json_args)` для вызова обработчика Rust
4. Разбирает JSON-конверт результата (`{"ok": value}` или `{"err": "message"}`)
5. Возвращает значение `ok` или выдаёт предупреждение для `err`

### Вызов PHP из Rust

Расширение предоставляет `oxphp_sapi_call_php()` — callback, который Rust может вызывать через мост для вызова PHP-функций:

1. Rust вызывает `oxphp_bridge_call_php(func_name, json_args)`
2. Мост вызывает `call_php_fn`, который установлен как `oxphp_sapi_call_php` во время MINIT
3. `oxphp_sapi_call_php` декодирует JSON-аргументы, вызывает `call_user_function()` и возвращает JSON-конверт
