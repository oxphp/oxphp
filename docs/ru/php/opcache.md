...
- **Bootstrap-состояние Worker Mode** — в [режиме Worker](../features/worker-mode.md) автозагрузчик, DI-контейнер и все объекты, построенные во внешней области, живут в памяти воркера. OPcache перекомпилирует изменённые классы, но воркер не выполнит повторно свой bootstrap. Для dev-цикла поставьте `WORKER_MAX_REQUESTS=1`, чтобы воркер пересоздавался после каждого запроса — это перезапускает внешнюю область и подхватывает все изменения.
