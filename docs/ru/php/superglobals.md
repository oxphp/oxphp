...
|---|---|---|
| **Traditional** (`INDEX_FILE` не задан) | Только когда URI содержит `.php/` и префикс скрипта существует на диске | Хвост после сегмента скрипта |
| **Framework** (`INDEX_FILE=index.php`) | **Всегда** — каждый запрос переписывается на фронт-контроллер | Полный оригинальный URI |
| **SPA** (`INDEX_FILE=index.html`) | Никогда — PHP вызывается только для точных `.php`-файлов, PATH_INFO нет | — |
...
Каждый нестатический запрос переписывается на `index.php`, при этом `PATH_INFO` несёт оригинальный URI. Ваш фронт-контроллер читает `PATH_INFO` для диспетчеризации маршрутов.
