...
`$_REQUEST` — это объединённый массив `$_GET`, `$_POST` и при необходимости `$_COOKIE`, формируемый PHP согласно INI-директиве `request_order` (по умолчанию: `"GP"` — GET, затем POST). Правила самого объединения OxPHP не меняет.
...
> **Worker-режим:** `$_REQUEST` пересобирается на каждый запрос. Обычно PHP строит его лениво и один раз — при первой загрузке скрипта, который его упоминает; в постоянном воркере это означало бы, что все последующие запросы читают параметры первого. OxPHP форсирует пересборку, поэтому объединённый массив всегда описывает обслуживаемый сейчас запрос.

---

## $_ENV

`$_ENV` содержит окружение процесса. В традиционном режиме поведение точно такое же, как в PHP-FPM: PHP заново наполняет массив из окружения на каждом запросе с учётом `variables_order`.

**Worker-режим его закрепляет.** Воркер выполняет бутстрап один раз, а `.env`-загрузчики (vlucas/phpdotenv, symfony/dotenv, `Env` в Laravel) пишут значения прямо в `$_ENV`, не трогая окружение процесса. Пересборка `$_ENV` на каждый запрос стирала бы конфигурацию приложения начиная со второго запроса, поэтому в worker-режиме массив сохраняется на всё время жизни воркера с момента появления — то, что записал бутстрап, видно каждому обслуженному этим воркером запросу.

```php
<?php
// бутстрап, до oxphp_worker()
Dotenv\Dotenv::createImmutable(__DIR__)->load();   // пишет в $_ENV

oxphp_worker(function () {
    echo $_ENV['DATABASE_URL'];  // на месте и на 10 000-м запросе
});
```

> **Размен:** раз `$_ENV` в worker-режиме никто не пересобирает, `filter_input(INPUT_ENV, …)`, `filter_input_array(INPUT_ENV)` и `filter_has_var(INPUT_ENV, …)` сообщают, что переменных нет. Читайте окружение через `getenv()` или напрямую из `$_ENV` — там и значения процесса, и добавленное бутстрапом. Это касается только worker-режима; традиционный, framework и SPA не затронуты.

Закрепление опирается на значение по умолчанию `auto_globals_jit=1`. При `auto_globals_jit=0` PHP наполняет `$_ENV` из окружения процесса на каждом запросе раньше, чем в это может вмешаться расширение, и значения `.env`-загрузчика в worker-режиме не сохранятся.
