...
> **`RUNTIME_HOOKS` не отменяет эту функцию.** `oxphp_sleep()` кооперативна в worker-режиме без всякой настройки и принимает дробные секунды. `RUNTIME_HOOKS=sleep` — отдельная опциональная накладка, которая заставляет кооперировать ещё и *нативные* вызовы `sleep()`/`usleep()` (например, в стороннем коде); необходимость в `oxphp_sleep()` для своего кода она не убирает. См. [Runtime-хуки](../operations/configuration.md).
