...
> **За reverse proxy:** `scheme()`, `isSecure()`, `host()` и `port()` учитывают `X-Forwarded-Proto` и `X-Forwarded-Host`, когда `TRUSTED_PROXIES` включает в себя пира соединения. Без доверенных прокси они отражают прямое соединение.
...
Возвращает IP-адрес клиента. Когда настроен `TRUSTED_PROXIES` и пир запроса входит в доверенное множество, это самый правый недоверенный адрес из `X-Forwarded-For` или RFC 7239 `Forwarded`. В противном случае это прямой IP-пир — обычно ваш балансировщик нагрузки, а не конечный клиент.

Сырой заголовок `X-Forwarded-For` по-прежнему доступен через `$request->header('X-Forwarded-For')` для продвинутых случаев, но разбирать его вручную редко получается корректно (leftmost против rightmost, отсутствие проверки доверия по CIDR). Лучше настройте `TRUSTED_PROXIES` — см. [Доверенные прокси](../security/trusted-proxies.md).
