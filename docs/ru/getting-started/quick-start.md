...
```bash
docker run -p 80:80 -v .:/var/www/html ghcr.io/oxphp/oxphp:0.8.0
```
...
```bash
docker run -p 80:80 -p 9090:9090 -e INTERNAL_ADDR=0.0.0.0:9090 -v .:/var/www/html ghcr.io/oxphp/oxphp:0.8.0
```
...
Официальный образ включает серверный бинарный файл, PHP 8.4 или 8.5 ZTS (по умолчанию 8.5; для 8.4 загрузите тег `:0.8.0-php8.4` или любой вариант `*-php8.4*`), PHP-расширение OxPHP и все зависимости времени выполнения.
