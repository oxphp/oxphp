...
`oxphp serve --user=<spec>` сворачивает это в один процесс: OxPHP занимает листенеры **под root**, а затем безвозвратно сбрасывает привилегии до целевого пользователя **до того**, как будет принято хоть одно соединение или запущен хоть один PHP-воркер. Вы получаете привилегированный порт *и* обработку запросов без root — без дополнительных capabilities.

Запускайте контейнер **под root** — не задавайте при этом `user:`, потому что для bind нужен root, — и передавайте флаг через `command`:
...
`<spec>` принимает имя пользователя, `name:group`, числовой `uid` или `uid:gid`. Сброс выполняет `initgroups → setgid → setuid`, проверяет, что root вернуть невозможно, и необратим; на Linux дополнительно ставится `no_new_privs`. Если процесс запущен **не** под root, `serve --user` завершается с ошибкой, а не продолжает молча работать под root.
