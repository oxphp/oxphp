...
> **Совет:** Готовая версия этого Dockerfile доступна в файле [`examples/dockerfile/Dockerfile`](../../../examples/dockerfile/Dockerfile) в репозитории. Скопируйте его в свой проект и настройте расширения под ваши нужды.
...
Контейнер стартует от root, после чего `oxphp serve` по умолчанию сбрасывает привилегии до `www-data` ещё до обслуживания трафика (см. заметку о безопасности ниже). Явно фиксируйте идентичность на уровне оркестратора, если нужен конкретный uid или пользователь, отличный от `www-data`.

### Лучшая практика (два этапа, меньший образ)

Для минимального итогового образа собирайте расширения в отдельном builder-этапе и копируйте в runtime-этап только готовые `.so`-файлы. Разбор [Многоэтапный Dockerfile](#многоэтапный-dockerfile) выше использует `FROM ghcr.io/oxphp/oxphp:0.10.0 AS prod` — простой, переносимый и рекомендуемый в качестве стартовой точки.
...
В продакшен-образе нет директивы `USER`, поэтому контейнер **стартует** от root (как и в конвенциях `nginx:alpine` / `php:*-fpm-alpine` / `frankenphp:alpine`). Старт от root позволяет OxPHP занять привилегированные порты — но это больше не значит, что трафик *обслуживается* от root: **`oxphp serve` и `oxphp run` по умолчанию сбрасывают привилегии до `www-data`**, занимая листенеры от root, а затем безвозвратно сбрасывая привилегии до того, как будет обработан хоть один запрос или запущен хоть один PHP-воркер. В официальном образе (где есть учётная запись `www-data`) это происходит «из коробки», без какой-либо настройки оркестратора.

Идентичность процесса всё ещё можно явно зафиксировать на уровне оркестратора — это рекомендуется, когда нужен конкретный uid, дополнительная defense-in-depth или пользователь, отличный от `www-data`:

- **Docker:** `docker run --user www-data ghcr.io/oxphp/oxphp:0.10.0`
- **Compose:**
  ```yaml
  services:
    oxphp:
...
Когда вы стартуете контейнер не от root таким образом, OxPHP уже непривилегирован, и дефолтный самосброс становится no-op — но тогда процесс не сможет занять порты ниже 1024 (см. [Запуск без root на порту 80](#запуск-без-root-на-порту-80-serve---user), чтобы сохранить привилегированный bind *и* обработку запросов без root). Чтобы намеренно продолжать обслуживать трафик от root, передайте `oxphp serve --user=root`.

Пользователь `www-data` (uid 82, gid 82) предсоздан базовым образом, а `/var/www/html` передаётся ему во владение при сборке — поэтому любой из путей сброса привилегий, включая дефолтный самосброс, приводит к читабельному webroot'у.

> CLI-вызовы вроде `docker run … php artisan migrate` запускают бинарник `php` напрямую, а не `oxphp serve`/`run`, поэтому они **не** делают самосброс — они работают от стартового пользователя контейнера (по умолчанию root). Для них используйте `--user` у Docker, как показано выше.

### Запуск без root на порту 80 (`serve --user`)

У сброса привилегий на уровне оркестратора (выше) есть одно ограничение: процесс, стартовавший как `www-data`, не может занять привилегированный порт (ниже 1024). Чтобы слушать `:80`/`:443`, иначе пришлось бы выдавать `CAP_NET_BIND_SERVICE`, использовать entrypoint в духе `su-exec` либо высокий порт (например `:8080`) за пробросом портов.

OxPHP сворачивает это в один процесс: он занимает листенеры **под root**, а затем безвозвратно сбрасывает привилегии **до того**, как будет принято хоть одно соединение или запущен хоть один PHP-воркер. Вы получаете привилегированный порт *и* обработку запросов без root — без дополнительных capabilities. **По умолчанию сброшенный пользователь — `www-data`**, поэтому в официальном образе достаточно просто стартовать контейнер от root, чтобы получить привилегированный bind, обслуживаемый `www-data`, — без всякого флага. `--user=<spec>` нужен только чтобы сбросить привилегии до *другого* пользователя; `--user=root` оставляет root.

Запускайте контейнер **под root** — не задавайте при этом `user:`, потому что для bind нужен root. Пример ниже передаёт `--user=www-data` явно для самодокументируемости, но он совпадает с дефолтом:
...
`<spec>` принимает имя пользователя, `name:group`, числовой `uid` или `uid:gid`. Сброс выполняет `initgroups → setgid → setuid`, проверяет, что root вернуть невозможно, и необратим; на Linux дополнительно ставится `no_new_privs`. **Явный** `--user` работает fail-fast: если процесс запущен **не** под root, `serve --user` завершается с ошибкой, а не продолжает молча работать под root. (Дефолтный сброс, наоборот, best-effort — при старте не от root он просто пропускается, ведь сбрасывать нечего.)

> Выбирайте **одну** модель, а не обе. Используйте сброс на уровне оркестратора (`user:` / `runAsUser`), когда `:80` терминирует высокий порт или внешний балансировщик. Используйте `serve --user`, когда хотите, чтобы привилегированным bind владел сам OxPHP. Одновременно заданные `user:` **и** `serve --user` ломают bind — контейнер уже не root.

**Чек-лист прав доступа для сброшенного пользователя.** После сброса всё, к чему OxPHP обращается во время работы, должно быть доступно `<spec>`:

| Ресурс | Требование |
|--------|------------|
| `DOCUMENT_ROOT` | Чтение. `/var/www/html` передаётся `www-data` при сборке образа, так что по умолчанию это выполнено. |
| Путь сохранения сессий (`session.save_path`, по умолчанию `/tmp`) | Запись. |
| Каталог временных загрузок (`upload_tmp_dir`) | Запись, если используются загрузки файлов. |
| Файловый кеш OPcache (`opcache.file_cache`) | Запись, если включён вторичный файловый кеш. |
| Файловый access-log | Запись, если лог пишется в файл, а не в stdout. |
| Приватный ключ TLS (`TLS_KEY`) | **Доступен на чтение сброшенному пользователю** — читаемый для группы или всех, а не root-only `0600`. Ключ читается *после* сброса, поэтому root-only ключ ломает старт TLS. |
