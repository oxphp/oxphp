...
| `QUEUE_CAPACITY` | `PHP_WORKERS * 128` | Максимальное количество запросов в очереди PHP. При заполнении новые PHP-запросы получают ответ `529 Site is overloaded`. Использует начальное количество воркеров для динамического режима |
...
| `ERROR_PAGES_DIR` | *(нет)* | Директория с HTML-файлами пользовательских страниц ошибок, именованными `{status}.html` (например, `404.html`, `503.html`, `529.html`) |
...
| `WORKER_MAX_MEMORY_MIB` | `0` | Максимальная память (в мегабайтах), которую воркер может использовать до рециклирования. `0` отключает лимит |
...
Режим воркера сохраняет PHP-процессы живыми между запросами. Скрипт воркера вызывает `oxphp_worker()` с callback-обработчиком. Воркеры автоматически рециклируются при достижении `WORKER_MAX_REQUESTS` или `WORKER_MAX_MEMORY_MIB`. Установите оба значения в `0` для отключения рециклирования.
