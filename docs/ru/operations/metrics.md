...
| `oxphp_pending_requests` | gauge | Принятые, но ещё не отвеченные PHP-запросы — ожидающие слот в очереди, стоящие в очереди или исполняющиеся. Только то, что направлено в PHP: статика, 404 и запрещённые пути отвечаются без очереди и сюда не попадают |
| `oxphp_dropped_requests_total` | counter | Запросы, для которых PHP-воркер завершился с ошибкой после принятия запроса |
| `oxphp_admission_refused_total` | counter | Запросы, отвеченные без участия воркера. Метка: `reason` — `wait_timeout` (прождал весь `QUEUE_WAIT_TIMEOUT_MS`, пулу нужен запас), `waiting_full` (уже `QUEUE_MAX_WAITING` ожидающих, поднимайте его или `MAX_CONNECTIONS`), `queue_full` (`QUEUE_WAIT_TIMEOUT_MS=0`, ожидание выключено), `shutting_down` (дедлайн graceful drain наступил, пока запрос ещё ждал допуска), `pool_unavailable` (передавать запрос некому — ни одного живого воркера не осталось; пул мёртв, а не занят). Перегрузка — только первые три, они отвечают 529; `shutting_down` отвечает 503, как и остальной graceful drain, `pool_unavailable` — 500. Алертить на перегрузку надо именно по первым трём: метрика целиком двигается и на рестарте. В `oxphp_queue_wait_us` не попадают |
...
| `oxphp_workers_idle` | gauge | Число воркер-потоков без запроса в работе, вычисляется как `workers_current - busy_workers` |
| `oxphp_busy_workers` | gauge | Число воркер-потоков, исполняющих хотя бы один запрос; никогда не превышает `oxphp_workers_current`. Считает потоки, а не запросы — в worker mode один поток мультиплексирует множество фиберов и считается один раз. Запросы, ожидающие допуска или стоящие в очереди, не учитываются — они видны в `oxphp_pending_requests` |
...
Метрика меряет именно ожидание — допуска, а затем в очереди — за вычетом времени исполнения скрипта, то есть отвечает на вопрос «сколько прошло до того, как запрос забрал воркер», а не «сколько занял запрос целиком». Запросы, отбитые с кодом 529, в очереди не стояли и сюда не попадают — их считает `oxphp_admission_refused_total`.

Высокое время ожидания означает, что все воркеры заняты и следует увеличить `PHP_WORKERS`. Диапазон доходит до секунды — ровно до дефолтного `QUEUE_WAIT_TIMEOUT_MS`, — так что запрос, простоявший почти весь бюджет и затем исполненный, попадает в измеримый бакет, а не в `+Inf`. Дольше бюджета не ждёт ничто из обслуженного: сверх него запрос отбивается. Единственное, что возвращает ожидания в `+Inf`, — поднятый `QUEUE_WAIT_TIMEOUT_MS`.
...
Значение всегда лежит между `0` и `1`. Устойчивая единица означает, что все воркеры заняты и новые запросы встают в очередь; смотрите вместе с `rate(oxphp_admission_refused_total{reason=~"queue_full|wait_timeout|waiting_full"}[5m])`, чтобы понять, превращается ли накопление в отказы, и с `oxphp_pending_requests`, чтобы увидеть его глубину.
