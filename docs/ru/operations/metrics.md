...
| `oxphp_request_cancelled_total` | counter | Отменённые запросы по причине. Метка: `reason` (`client_abort`, `timeout`, `shutdown`). Публикуется всегда |
...
Наблюдаемость на уровне отдельных воркеров, публикуемая супервизором воркеров. Каждый ряд несёт метку `worker_id` (индекс слота). Появляются, когда супервизор начинает отслеживать состояние по воркерам.

| Метрика | Тип | Описание |
|--------|-----|---------|
| `oxphp_worker_request_age_seconds` | gauge | Возраст выполняющегося запроса на каждом воркере, в секундах. Метка: `worker_id` |
| `oxphp_worker_long_running_total` | counter | Сканы супервизора, обнаружившие запрос старше порога зависания. Метка: `worker_id` |
| `oxphp_worker_stuck_total` | counter | Счётчик классификации зависаний по воркерам. Метки: `worker_id`, `kind` (`io`, `c_call`, `cpu`) |
...
Эти метрики требуют, чтобы `ASYNC_WORKERS` был установлен в ненулевое значение, и у каждой свой шлюз публикации: счётчики появляются только после того, как хотя бы одна задача была отправлена или отклонена, gauge-метрики `_in_flight` / `_in_flight_limit` появляются, когда пул подключит свой in-flight-счётчик, а `oxphp_async_output_discarded_bytes_total` появляется только после того, как часть вывода была отброшена.
