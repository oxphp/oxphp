...
Сигнатура замыкания — `function (mixed &$value): mixed` — `$value` передаётся по ссылке, так что вы мутируете его на месте. Нормальное возвращаемое значение замыкания пробрасывается вызывающему `withLock` / `tryWithLock` / `withLockTimeout`. **Возвратный путь поддерживает скаляры, `null` и экземпляры `Shared\*`** (string, int, float, bool, byte-string, null и любой handle, реализующий `OxPHP\Shared\Shareable`). Возврат PHP-массива поднимает `OxPHP\Shared\TypeException` — этот случай пока не поддерживается и отслеживается отдельно. Чтобы пробросить структурированное состояние из массива, либо мутируйте `&$value` на месте и перечитайте его после вызова, либо подготовьте нужное в переменной через `use (&$captured)`.
...
- **Return замыкания покрывает скаляры, `null` и экземпляры `Shared\*`; массивы пока не поддерживаются.** Сохранённое значение может оставаться массивом (мутируйте через `&$value`), но сам *return* замыкания принимает string/int/float/bool/null/byte-string и любой handle `OxPHP\Shared\Shareable`. Возврат PHP-массива поднимает `OxPHP\Shared\TypeException`. Обход для массивов: захватите в `use (&$x)`-переменную или прочитайте состояние через последующий `withLock`, возвращающий скалярную проекцию.
