...
8. **Compression** — text-based responses are compressed before being sent, with whichever of Brotli and gzip the client's `Accept-Encoding` ranks highest (configurable via `COMPRESSION_LEVEL` and `GZIP_LEVEL`).
