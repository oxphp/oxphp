...
A request that arrives at a full queue is not rejected outright — it waits for a slot for up to `QUEUE_WAIT_TIMEOUT_MS` (default: 1000 ms) and is admitted as soon as a worker picks up the request ahead of it. That setting is the ceiling on the wait rather than its length: the server shortens the budget while the work the waiting admits is finishing for clients who have already gone, and restores it once the queue drains, so under overload the deadline stamped on an arrival is the shortened one. Waiting requests are admitted in arrival order. The budget is a single deadline stamped on arrival and it follows the request into the queue: when it passes the request is answered with a 529, and a worker that reaches one past it refuses rather than executes it. Where the client has closed the connection by then the answer is a `499` instead: there is nobody left to receive the refusal, and counting one would make the overload series a statement about how long clients were prepared to wait. The budget therefore bounds the whole wait rather than its admission half — which matters because the queue is deep enough that reaching its tail on a slow pool takes far longer than any budget an operator would set, and because a queue nothing is being taken off has no pickup to enforce anything at. Load is therefore shed on elapsed wait time rather than on the queue depth at the instant the request arrived: a burst that drains in microseconds is served, while a pool that genuinely cannot keep up still sheds.

What the budget bounds is waiting, not execution. A request a worker picked up while its deadline still had time left is left alone however long the handler then runs — the check is what the request is waiting for, not how long it has taken — so response time under load is the wait plus execution time, and the budget is a ceiling on the first term only. It is a ceiling on the whole of that term, though: a request still waiting when its deadline passes is answered then, whether it is waiting for a queue slot or sitting in the queue for a worker.

`QUEUE_MAX_WAITING` caps how many requests may be waiting at once (default: initial workers × 128, capped at half of `MAX_CONNECTIONS`); past it, admission goes back to rejecting immediately. Waiting is not free — a waiting request holds its connection, and its already-buffered body, until it is admitted or the budget runs out — so an uncapped waiting set would consume every connection permit under sustained overload, block the accept loop, and leave the server dropping new connections instead of answering them. The default approximates how many requests the pool can actually admit inside the budget: waiting past that only defers a rejection while holding those resources. The `MAX_CONNECTIONS` / 2 ceiling bounds the waiting set alone, which is not by itself headroom for the accept loop: a queued request holds its connection until a worker reaches it or its budget runs out, and a running one holds it too — its queue slot was released at pickup — while `QUEUE_CAPACITY` has no connection-derived ceiling at all. So `PHP_WORKERS` + `QUEUE_CAPACITY` + `QUEUE_MAX_WAITING` has to stay below `MAX_CONNECTIONS`, and once it reaches the budget the accept loop parks with every permit taken — a client arriving then gets no response rather than a 529. The server warns about that at startup, and reports the park itself while it is happening: a `WARN` when the loop parks and an `INFO` with the duration when it resumes — rate-limited, so a server flapping at the ceiling does not turn its own log into the outage — plus the `oxphp_accept_stalled` gauge and the `oxphp_accept_stalls_total` counter, which are updated on every park, including the ones the rate limit passes over, and are served by the internal listener, outside this budget; see the [metrics reference](../operations/metrics.md). Clearing the startup warning is necessary rather than sufficient: connections that never reach PHP hold permits too, and a park they cause is one the startup check cannot predict. Note that the terms are sized in connections while the budget is spent by requests: over HTTP/2 one connection carries many requests, so an h2-heavy deployment reaches the cap from a fraction of its connection budget, should set `QUEUE_MAX_WAITING` explicitly, and can legitimately sit above the sum.

The waiting set is bounded twice, because a cap counted in requests says nothing about the memory they hold: the same number of waiters costs nothing on bodyless `GET`s and gigabytes on uploads. `QUEUE_MAX_WAITING_BYTES` (default 64 MiB) bounds the request-body bytes parked at once — past it a request carrying a body is refused immediately rather than parked, while bodyless ones keep waiting normally. Two things it does not cover, both of which predate the wait budget: bodies already handed to the queue, bounded in requests by `QUEUE_CAPACITY` and not in bytes, and the memory a body occupies while it is still being read off the connection, which no aggregate limit bounds at all.
...
8. **Compression** — text-based responses are compressed before being sent, with whichever of Brotli, Zstandard, and gzip the client's `Accept-Encoding` ranks highest; on a tie, cached static files get Brotli and everything else gets Zstandard (configurable via `COMPRESSION_ENCODINGS` and the per-coding level variables).
