...
- **Three explicit wait policies** instead of one overloaded `?float $timeout`:
  - `withLock($fn)` — block until the lock is acquired, with no bound of its own; the wait ends early only when the request or async task it runs in is being ended.
...
The closure signature is `function (mixed &$value): mixed` — `$value` is passed by reference, so you mutate it in place. The closure's normal return value is forwarded to the caller of `withLock` / `tryWithLock` / `withLockTimeout`. **The return path supports scalars, `null`, and `Shared\*` instances** (string, int, float, bool, byte-string, null, and any handle implementing `OxPHP\Shared\Shareable`). Returning a PHP array raises `OxPHP\Shared\TypeException` — that case is not yet supported and is tracked separately. To bubble structured array state up, either mutate `&$value` in place and re-read it after the call, or stage what you need in a `use (&$captured)` variable.

| Method                | Behaviour                                                       |
|-----------------------|-----------------------------------------------------------------|
| `withLock($fn)`       | Block until acquired, then run the closure. Waits as long as the request lives. |
...
- **Closure return covers scalars, `null`, and `Shared\*` instances; arrays are not yet supported.** The stored value can still be an array (mutate it via `&$value`), but the closure's own *return* path accepts string/int/float/bool/null/byte-string and any `OxPHP\Shared\Shareable` handle. Returning a PHP array raises `OxPHP\Shared\TypeException`. Workaround for arrays: capture into a `use (&$x)` variable, or read the state through a follow-up `withLock` that returns a scalar projection.
...
- **A `withLock` wait lasts as long as the request, and no longer.** When `max_execution_time` runs out, the server shuts down past its drain deadline with the request still running, or the async task the call runs in is cancelled while it waits, a `withLock` still waiting for the lock stops waiting, and the request ends the way that cancellation ends it anywhere else: a timeout is the same `504` fatal, not an exception your code can catch, while a cancelled async task sees `Async\AsyncException`. On a timeout the log — and the response, where `display_errors` is on — also carries a warning `Uncaught OxPHP\Async\AsyncException: Mutex::withLock: wait abandoned, the request is being ended` just ahead of the timeout fatal: the wait leaves by throwing that exception, and PHP reports an exception still in flight when a fatal error arrives. A client disconnecting does not stop the wait. With `max_execution_time = 0` and no shutdown, nothing does — use `withLockTimeout` where the wait needs a bound of its own.
- **A fatal error inside the closure leaves the lock locked.** A fatal — `max_execution_time` running out while the closure runs included — ends the request without the release step, so the mutex stays locked for the life of the process: a later `withLock` on it from another worker thread waits until its own request is ended, and one from the worker thread that ran the closure throws `DeadlockException`, because that thread still counts the mutex as held. Exceptions are not affected: a closure that throws releases the lock as described above. Keep closures short enough that the request's time limit never runs out inside one.
